...
export function readDirectSessionFollowPolicyV1(value: unknown): DirectSessionFollowPolicyV1 | null {
  ...
  return {
    ...
    ...(normalizeOptionalToken(candidate.generation) ? { generation: normalizeOptionalToken(candidate.generation) } : {}),
  };
}
...
export function buildDirectSessionFollowPolicyV1(
  ...
  return {
    ...
    policy: value.policy,
    ...(updatedAtMs !== undefined ? { updatedAtMs } : {}),
    ...(normalizeOptionalToken(value.generation) ? { generation: normalizeOptionalToken(value.generation) } : {}),
  };
}
