...
export interface Settings {
  /** 产品密码入口的来源账号绑定；退出只清凭据，保留此归属防止历史换号转交。 */
  passwordAccountBinding?: { accountId: string; loginName: string; serverKey: string }
...
/** 固定调用开始时的凭据路径，异步写盘不追随随后切换的服务器。 */
export async function writeCredentialsLegacy(credentials: { secret: Uint8Array, token: string }): Promise<void> {
  const primaryPath = configuration.privateKeyFile;
  const legacyPath = configuration.legacyPrivateKeyFile;
  const migrateLegacy = configuration.activeServerId === 'cloud' && legacyPath !== primaryPath;
  await ensureHappyHomeDirExists();
  await writeFile(primaryPath, JSON.stringify({
    secret: encodeBase64(credentials.secret), token: credentials.token
  }, null, 2), { mode: 0o600 });
  await bestEffortChmod(primaryPath, 0o600);
  if (migrateLegacy && existsSync(legacyPath)) await unlink(legacyPath).catch(() => {});
}
