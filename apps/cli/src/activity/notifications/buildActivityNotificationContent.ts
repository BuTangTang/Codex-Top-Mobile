...
export function buildActivityNotificationContent(
  ...
  if (event.directSession || event.topic === 'session_failed') {
    return {
      title: event.sessionTitle ?? 'Codex',
      body: event.topic === 'ready' ? 'The turn has completed.' : event.topic === 'session_failed' ? 'The turn failed.' : 'Codex Desktop needs your input.',
      data: { sessionId: event.sessionId, topic: event.topic, ...event.directSession, interaction: 'open_only',
