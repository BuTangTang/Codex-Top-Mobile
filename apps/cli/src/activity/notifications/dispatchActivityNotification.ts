...
function isTopicEnabled(channel: {
  ...
  if (topic === 'session_failed') return channel.topics.sessionFailed !== false;
...
export async function dispatchActivityNotificationAsync(params: Readonly<{
...
}>): Promise<Readonly<{ attemptedChannels: number; deliveredChannels: number; suppressedChannels: number; unknownChannels?: number; deliveryReason?: string }>> {
  ...
  for (const channel of channels) {
    ...
    try {
      if (channel.kind === 'expo_push') {
        if (!params.expoPushSender) { deliveryReason = 'sender_unavailable'; continue; }
        const result = await sendExpoPushActivityNotificationAsync({
          channel,
          event: params.event,
          sender: params.expoPushSender,
        });
        // 保留旧字段名称；这里只计入供应商接受，绝不宣称设备送达。
        if (result.status === 'submitted') deliveredChannels += 1;
        if (result.status === 'unknown') unknownChannels += 1;
  ...
  return {
    ...
    ...(unknownChannels > 0 ? { unknownChannels } : {}),
    ...(params.event.directSession && deliveryReason ? { deliveryReason } : {}),
  };
}
