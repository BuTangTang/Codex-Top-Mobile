...
export class ApiMachineClient {
    ...
    setRPCHandlers({
        ...
        const direct = machineRpcLifecycleRegistration.directSessions;
        if (direct && (deps?.directSessionNotifications ?? this.directSessionNotifications)) {
            // 复用机器连接和账号 update 生命周期；退出时先解绑，防止清理后重新恢复关注。
            const reportFollowFailure = () => logger.debug('[directSessions] Background follow reconciliation unavailable');
            const offConnection = this.onConnectionStateChange((state) => {
                void (state.phase === 'online' ? direct.reconcile() : direct.suspend()).catch(reportFollowFailure);
            });
            const offUpdate = this.onUpdate((update) => {
                const body = update.body;
                if (body.t === 'new-session' || (body.t === 'update-session' && body.metadata)) {
                    void direct.reconcile(body.id).catch(reportFollowFailure);
                } else if (body.t === 'delete-session') {
                    void direct.reconcile(body.sid).catch(reportFollowFailure);
                }
            });
            this.rpcLifecycleRegistrations.push({ dispose: async () => {
                offConnection(); offUpdate(); await machineRpcLifecycleRegistration.dispose();
            } });
        } else this.rpcLifecycleRegistrations.push(machineRpcLifecycleRegistration);
    }
