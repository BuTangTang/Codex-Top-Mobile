...
describe('authorizeMachineRpcRequest', () => {
  // 桌面发送沿用既有鉴权层；旧中转缺少授权或授权与密文目标不一致时都拒绝。
  it('binds external-owner sends to the exact authorized session', () => {
    const request = {
      method: `machine-1:${RPC_METHODS.DAEMON_DIRECT_SESSION_SEND}`,
      params: { sessionId: 's1' },
    };
    expect(authorizeMachineRpcRequest(request)).toMatchObject({ ok: false, errorCode: RPC_ERROR_CODES.FORBIDDEN });
    expect(authorizeMachineRpcRequest({ ...request, authorization: { kind: 'session.write', sessionId: 's2' } }))
      .toMatchObject({ ok: false, errorCode: RPC_ERROR_CODES.FORBIDDEN });
    expect(authorizeMachineRpcRequest({ ...request, authorization: { kind: 'session.write', sessionId: 's1' } }))
      .toEqual({ ok: true });
  });
