...
export type MachineRpcLifecycleRegistration = Readonly<{
  directSessions?: import('./rpcHandlers.directSessions').DirectSessionsRpcLifecycle;
...
/** 注册机器级 RPC，向各能力传入可信运行依赖，并统一返回生命周期清理入口。 */
export function registerMachineRpcHandlers(params: Readonly<{
  ...
  const directSessions = registerMachineDirectSessionsRpcHandlers({
    ...
    getDaemonIdentity: params.deps?.getActionOperationScope,
    notifications: params.deps?.directSessionNotifications,
