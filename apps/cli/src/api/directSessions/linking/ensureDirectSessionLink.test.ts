...
describe('ensureDirectSessionLink', () => {
  ...
  /** 构造已有来源链接，全部标题和身份均为合成数据，不访问真实会话。 */
  function existingTitleLink(name = '旧的首段正文', summary?: { text: string; updatedAt: number }) {
    const source = { kind: 'codexHome' as const, home: 'user' as const };
    const tag = `direct:v1:${sha256Hex('machine_1|codex|thread_title|codexHome:user:::')}`;
    const metadata = { tag, name, ...(summary ? { summary } : {}), codexSessionId: 'thread_title',
      directSessionV1: { v: 1, providerId: 'codex', machineId: 'machine_1', remoteSessionId: 'thread_title', source } };
    fetchSessionsPageMock.mockResolvedValue({ sessions: [{ id: 'same-session', metadata }], hasNext: false, nextCursor: null });
    fetchSessionByIdMock.mockResolvedValue({ id: 'same-session', metadata });
    tryDecryptSessionMetadataMock.mockImplementation(({ rawSession }: { rawSession: { metadata?: unknown } }) => rawSession.metadata);
    return { metadata, request: { credentials: { token: 'synthetic', encryption: { type: 'legacy' as const, secret: new Uint8Array([1]) } },
      machineId: 'machine_1', providerId: 'codex' as const, remoteSessionId: 'thread_title', source } };
  }

  it('refreshes an existing source title while reusing the linked session', async () => {
    const { metadata, request } = existingTitleLink();
    const result = await ensureDirectSessionLink({ ...request, titleHint: '来源短标题' });
    expect(result).toMatchObject({ sessionId: 'same-session', created: false });
    expect(getOrCreateSessionByTagMock).not.toHaveBeenCalled();
    expect(updateSessionMetadataWithRetryMock).toHaveBeenCalledTimes(1);
    expect(updateSessionMetadataWithRetryMock.mock.calls[0][0].updater(metadata)).toEqual({ ...metadata, name: '来源短标题' });
  });

  it('preserves manual and concurrently renamed summaries when refreshing the source name', async () => {
    const { metadata, request } = existingTitleLink('旧来源标题', { text: '我的自定义名', updatedAt: 1 });
    await ensureDirectSessionLink({ ...request, titleHint: '来源短标题' });
    expect(updateSessionMetadataWithRetryMock).toHaveBeenCalledTimes(1);
    const updater = updateSessionMetadataWithRetryMock.mock.calls[0][0].updater;
    expect(updater(metadata)).toEqual({ ...metadata, name: '来源短标题' });
    const concurrent = { ...metadata, summary: { text: '并发修改后的自定义名', updatedAt: 2 }, extra: 'preserve' };
    expect(updater(concurrent)).toEqual({ ...concurrent, name: '来源短标题' });
    const otherSource = { ...concurrent, directSessionV1: { ...metadata.directSessionV1,
      source: { kind: 'codexHome', home: 'user', homePath: '/synthetic/other' } } };
    expect(updater(otherSource).name).toBe('旧来源标题');
  });

  it.each([undefined, '来源短标题'])('does not write metadata for absent or unchanged title hint %s', async (titleHint) => {
    const { request } = existingTitleLink('来源短标题');
    await ensureDirectSessionLink({ ...request, titleHint });
    expect(updateSessionMetadataWithRetryMock).not.toHaveBeenCalled();
  });

  it('keeps the existing name protection policy for another provider', async () => {
    const source = { kind: 'piAgentDir' as const, agentDir: '/synthetic/pi' };
    const tag = `direct:v1:${sha256Hex('machine_1|pi|thread_title|piAgentDir:/synthetic/pi')}`;
    const metadata = { tag, name: 'Existing provider title', piSessionId: 'thread_title',
      directSessionV1: { v: 1, providerId: 'pi', machineId: 'machine_1', remoteSessionId: 'thread_title', source } };
    fetchSessionsPageMock.mockResolvedValue({ sessions: [{ id: 'same-session', metadata }], hasNext: false, nextCursor: null });
    fetchSessionByIdMock.mockResolvedValue({ id: 'same-session', metadata });
    tryDecryptSessionMetadataMock.mockImplementation(({ rawSession }: { rawSession: { metadata?: unknown } }) => rawSession.metadata);
    await ensureDirectSessionLink({ credentials: { token: 'synthetic', encryption: { type: 'legacy', secret: new Uint8Array([1]) } },
      machineId: 'machine_1', providerId: 'pi', remoteSessionId: 'thread_title', source, titleHint: 'New provider title' });
    expect(updateSessionMetadataWithRetryMock).not.toHaveBeenCalled();
  });

  it('does not replace the name of an imported persisted session', async () => {
    const { metadata, request } = existingTitleLink('导入后保留的标题');
    const imported = { ...metadata, directSessionV1: undefined, externalHistoryImportV1: {
      v: 1, providerId: 'codex', remoteSessionId: 'thread_title', importedAtMs: 1, source: request.source } };
    fetchSessionsPageMock.mockResolvedValue({ sessions: [{ id: 'same-session', metadata: imported }], hasNext: false, nextCursor: null });
    fetchSessionByIdMock.mockResolvedValue({ id: 'same-session', metadata: imported });
    await ensureDirectSessionLink({ ...request, titleHint: '来源短标题' });
    expect(updateSessionMetadataWithRetryMock).not.toHaveBeenCalled();
  });
