...
/** 用户重新链接时刷新来源信息；手动 summary 与并发改名始终由现有元数据保留。 */
function resolveRefreshedDirectSessionMetadata(params: Readonly<{
  ...
  // 仅修复 Codex 的来源名称同步；其他提供方保留既有名称保护策略，导入会话也不覆盖。
  // CAS 重试会以最新元数据再次核对，summary 保持原样并继续优先作为用户显示名。
  const existingDirect = asMetadataRecord(params.currentMetadata.directSessionV1);
  const existingSource = asMetadataRecord(existingDirect?.source);
  const matchesDirectIdentity = Boolean(identity?.providerId === 'codex' && existingDirect && existingSource
    && existingDirect.providerId === identity.providerId
    && existingDirect.machineId === identity.machineId
    && existingDirect.remoteSessionId === identity.remoteSessionId
    && resolveSourceKey(identity.providerId, existingSource as DirectSessionsSource) === resolveSourceKey(identity.providerId, identity.source));
  if (titleHint && (!currentTitle || matchesDirectIdentity)
    && normalizeNullableString(params.currentMetadata.name) !== titleHint) {
