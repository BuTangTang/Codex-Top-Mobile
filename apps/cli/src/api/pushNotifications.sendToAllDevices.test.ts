...
describe('PushNotificationClient.sendToAllDevicesAsync', () => {
  ...
  it('does not count an account without push tokens as delivery through the real dispatcher', async () => {
    vi.mocked(axios.get).mockResolvedValue({ data: { tokens: [] } });
    const result = await dispatchActivityNotificationAsync({
      settings: accountSettingsParse({}),
      expoPushSender: new PushNotificationClient('t', 'https://api.example.test'),
      event: { topic: 'ready', sessionId: 'linked', waitingForCommandLabel: 'Codex' },
    });
    expect(result.deliveredChannels).toBe(0);
    expect(sendPushNotificationsAsyncSpy).not.toHaveBeenCalled();
  });

  it('keeps a dropped Expo submission unknown and does not resubmit it', async () => {
    vi.mocked(axios.get).mockResolvedValue({ data: { tokens: [{ id: '1', token: 'ExponentPushToken[a]' }] } });
    sendPushNotificationsAsyncSpy.mockRejectedValueOnce(new Error('connection dropped after submission'));
    const result = await new PushNotificationClient('t').sendToAllDevicesAsync('Title', 'Body', { sessionId: 'linked' });
    expect(result).toMatchObject({ status: 'unknown' });
    expect(sendPushNotificationsAsyncSpy).toHaveBeenCalledTimes(1);
  });

  it.each([{ tickets: [] }, { tickets: [{ status: 'ok' }] }, { tickets: [{ status: 'unexpected' }] }])('does not retry an ambiguous Expo ticket response: $tickets', async ({ tickets }) => {
    sendPushNotificationsAsyncSpy.mockResolvedValueOnce(tickets);
    const result = await new PushNotificationClient('t').sendPushNotifications([{ to: 'ExponentPushToken[a]', body: 'Body' }]);
    expect(result.status).toBe('unknown');
    expect(sendPushNotificationsAsyncSpy).toHaveBeenCalledTimes(1);
  });

  it('submits Desktop requests as open-only without Happier approval actions', async () => {
    vi.mocked(axios.get).mockResolvedValue({ data: { tokens: [{ id: '1', token: 'ExponentPushToken[a]' }] } });
    sendPushNotificationsAsyncSpy.mockResolvedValueOnce([{ status: 'ok', id: 'ticket-1' }]);
    const result = await new PushNotificationClient('t').sendToAllDevicesAsync('Title', 'Body', {
      sessionId: 'linked', requestId: 'native-request', kind: 'permission', interaction: 'open_only',
    });
    expect(result).toMatchObject({ status: 'submitted' });
    expect(sendPushNotificationsAsyncSpy.mock.calls[0][0][0].categoryId).toBeUndefined();
  });
