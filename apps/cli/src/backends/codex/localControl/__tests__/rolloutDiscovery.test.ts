...
describe('codex local-control rollout discovery', () => {
    ...
    it('applies scanLimit before statting rollout history', async () => {
        ...
        try {
            ...
            expect(discovered?.filePath).toBe(fresh);
            // 元数据 reader 可以再次 stat 同一文件；scanLimit 限制的是候选历史范围。
            expect(new Set(rolloutDiscoveryStatPaths).size).toBeLessThanOrEqual(5);
        } finally {
            await rm(root, { recursive: true, force: true });
        }
    });
    ...
    it('does not use metadata after an invalid first line to establish rollout ownership', async () => {
        const root = await mkdtemp(join(tmpdir(), 'codex-sessions-invalid-first-line-'));
        try {
            const filePath = join(root, 'rollout-2026-02-04T12-00-00-copied.jsonl');
            await writeFile(filePath, `invalid-json\n${sessionMetaLine({
                id: 'copied', timestamp: '2026-02-04T12:00:01.000Z', cwd: '/synthetic',
            })}\n`);
            expect(await discoverCodexRolloutFileOnce({
                sessionsRootDir: root,
                startedAtMs: Date.parse('2026-02-04T12:00:00.000Z'),
                cwd: '/synthetic',
                scanLimit: 10,
            })).toBeNull();
        } finally {
