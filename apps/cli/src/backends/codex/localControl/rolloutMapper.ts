...
export type CodexRolloutAction =
    ...
    | { type: 'user-text'; text: string; clientId?: string }
...
/** 保留原生用户消息的文本和有效客户端 ID，并沿用系统注入内容的过滤规则。 */
function mapNativeUserMessage(text: string | null, clientId: unknown): CodexRolloutAction[] {
    if (!text || shouldFilterHarnessBlob(text)) return [];
    return [{
        type: 'user-text',
        text,
...
/** 解析单条 Codex rollout；用户消息按显式历史格式选源，其余事件统一映射为语义动作。 */
export function mapCodexRolloutEventToActions(event: unknown, opts: {
    debug: boolean;
    ...
    if (env.type === 'event_msg') {
        ...
        // Codex 0.155 的显式 history_mode 决定用户消息的唯一来源，不能与 response_item 猜配。
        if (opts.historyMode === 'legacy' && payloadType === 'user_message') {
            return mapNativeUserMessage(
                typeof payload.message === 'string' ? payload.message : null,
                payload.client_id,
            );
        }
        if (opts.historyMode === 'paginated' && payloadType === 'item_completed') {
            const item = asRecord(payload.item);
            if (item?.type === 'UserMessage') {
                return mapNativeUserMessage(readCodexMessageContentText(item.content), item.client_id);
            }
        }
