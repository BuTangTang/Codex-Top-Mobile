import type { DirectSessionsSource, DirectTranscriptRawMessageV1, DirectTranscriptTruncationReason, DirectTranscriptHistoryAvailability } from '@happier-dev/protocol';
...
export async function pageCodexTranscript(params: Readonly<{
  ...
  if (bestHome === null) {
    ...
    return { items: previewItem ? [previewItem] : [], nextCursor: null, tailCursor, hasMore: false,
      historyAvailability: previewItem ? 'preview_only' : 'unavailable' };
  }
  ...
  try {
    ...
    // 发现文件之后仍可能被删除或失去读取权限；不把失败投影成历史末页。
    return { items: [], nextCursor: params.cursor ?? null, tailCursor: null, hasMore: true, historyAvailability: 'unavailable' };
  }
}
