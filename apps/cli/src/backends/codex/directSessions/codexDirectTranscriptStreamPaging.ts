...
async function readUserMessageHistoryMode(filePath: string): Promise<CodexRolloutHistoryMode | undefined> {
  // 只认文件首个 session_meta 的显式声明；旧文件缺少声明时保留原有 offset 投影。
  // 固定契约：openai/codex 0e2f848bf4a4e8d41a02d848a851ba126c09d185，rollout/src/policy.rs。
  const meta = await readCodexSessionMetaFromRollout(filePath);
  return meta?.history_mode === 'legacy' || meta?.history_mode === 'paginated' ? meta.history_mode : undefined;
}
...
/** 将各流已消费进度和文件边界编码为当前版本游标，并标记需要重新建立基线的流。 */
...
/** 为各 rollout 当前文件尾部建立前向游标，作为后续追加读取的起点。 */
...
/** 只接受当前投影版本的持久化流游标，防止混用不同来源的用户消息身份。 */
function decodeDurableStreamVectorCursor(
  ...
  // v5 使用 response_item 的用户消息 offset，不能与当前原生事件投影混合。
  if (!cursor || cursor.kind !== 'codexForwardStreamVector' || cursor.v !== 7) return null;
  return new Map(cursor.streams.map((entry) => [entry.fileRelPath, entry]));
}

/** 从各流断点读取完整追加记录，校验文件连续性，并把新发现的子任务纳入本次读取。 */
async function collectReadAfterRecords(params: Readonly<{
  ...
  for (let queueIndex = 0; queueIndex < streamQueue.length; queueIndex += 1) {
    ...
    const historyMode = await readUserMessageHistoryMode(stream.filePath);
    const page = await readJsonlFileForward({
      strictRead: true,
...
/** 合并会话及子任务的历史窗口，返回可继续分页和追尾的独立流游标。 */
export async function pageCodexRolloutStreams(params: Readonly<{
  ...
  const parsedCursor = decodeCodexDirectBackwardCursor(params.cursor);
  // 投影变化后由现有 source_discontinuity 路径替换旧气泡，避免跨版本重复展示。
  const decoded = parsedCursor?.v === 6 ? parsedCursor : null;
