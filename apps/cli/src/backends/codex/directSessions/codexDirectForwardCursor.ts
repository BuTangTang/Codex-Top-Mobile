...
type CodexDurableStreamVectorForwardCursor = Readonly<{
  v: 5 | 6 | 7;
  kind: 'codexForwardStreamVector';
...
export function decodeCodexDirectForwardCursor(raw: string): CodexDirectForwardCursor | null {
  ...
  try {
    ...
    if ((record.v === 5 || record.v === 6 || record.v === 7) && record.kind === 'codexForwardStreamVector') {
      const rawStreams = Array.isArray(record.streams) ? record.streams : [];
      const streams = rawStreams
        .map((entry) => {
          if (!entry || typeof entry !== 'object' || Array.isArray(entry)) return null;
      ...
      return { v: record.v, kind: 'codexForwardStreamVector', streams };
    }
    return null;
  } catch {
    return null;
