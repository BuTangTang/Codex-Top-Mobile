...
type CodexBackwardProvenanceStreamCursor = Readonly<{
  v: 4 | 5 | 6;
  kind: 'codexBackwardStreamVector';
  streams: readonly Readonly<{
    fileRelPath: string;
...
export type CodexDirectBackwardCursor = CodexBackwardStreamCursorV3 | CodexBackwardProvenanceStreamCursor;

export function encodeCodexDirectBackwardCursor(value: CodexDirectBackwardCursor): string {
  return Buffer.from(JSON.stringify(value), 'utf8').toString('base64url');
}

/** 校验历史分页游标中的流位置与子任务来源，保留版本供分页入口判断兼容性。 */
export function decodeCodexDirectBackwardCursor(raw: string | undefined): CodexDirectBackwardCursor | null {
  ...
  try {
    ...
    if ((record.v !== 3 && record.v !== 4 && record.v !== 5 && record.v !== 6) || record.kind !== 'codexBackwardStreamVector') return null;
    ...
    return { v: record.v, kind: 'codexBackwardStreamVector', streams: durableStreams };
  } catch {
    return null;
  }
}
