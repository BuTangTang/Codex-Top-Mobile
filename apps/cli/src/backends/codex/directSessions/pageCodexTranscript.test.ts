...
describe('pageCodexTranscript', () => {
  it('reports unavailable when neither a rollout nor preview exists', async () => {
    ...
    await expect(pageCodexTranscript({ source: { kind: 'codexHome', home: 'user' },
      activeServerDir: root, env: createCodexAppServerProcessEnv(fakeAppServer, { CODEX_HOME: codexHome }),
      remoteSessionId: 'missing-history', direction: 'older', maxBytes: 4096, maxItems: 10,
    })).resolves.toMatchObject({ items: [], historyAvailability: 'unavailable' });
  });
  ...
  it('pages newest-first (direction=older) from a rollout jsonl transcript', async () => {
    ...
    expect(first).toMatchObject({ historyAvailability: 'available' });
  ...
  it('falls back to app-server preview metadata when rollout files are missing', async () => {
    ...
    expect(result).toMatchObject({ historyAvailability: 'preview_only' });
  });
