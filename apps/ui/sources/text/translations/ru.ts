...
export const ru: TranslationStructure = {
  ...
  settings: {
    ...
    openSourceMaterials: "Материалы проекта с открытым кодом",
    accountAndConnection: "Аккаунт и подключение",
    ...
    aboutFooter:
      "Codex Top основан на проекте Happier с открытым исходным кодом.",
    whatsNew: "Что нового",
  ...
  phonePasswordLogin: {
      ...
      invalidCredentials: "Неверный аккаунт или пароль, либо доступ недоступен.",
      rateLimited: "Слишком много попыток. Повторите позже.",
      unavailable: "Не удалось войти. Проверьте сервер и повторите попытку.",
  },
  sessionsList: {
      statusAwaitingConfirmation: "Ожидает подтверждения",
    serverHeader: ({ server }: { server: string }) => `Сервер: ${server}`,
    storagePersistedTab: "Облачные сеансы",
    storageDirectTab: "Сеансы на компьютере",
  ...
  directSessions: {
    browseListChanged: "Список сессий изменился. Обновите его, чтобы продолжить загрузку.",
    browseListIncomplete: "В этом списке пока представлены не все сессии. Возможно, доступны и другие. Обновите список и повторите попытку позже.",
    historyPreviewOnly: "Доступен только предварительный просмотр. Полную историю сессии невозможно прочитать из текущего источника.",
    historyUnavailable: "История сессии временно недоступна. Ранее загруженные сообщения сохранены.",
    historyAvailabilityUnknown: "Текущий источник ещё не сообщил, доступна ли история сессии.",
        followNotifications: "Включить уведомления о задаче",
        unfollowNotifications: "Отключить уведомления о задаче",
        followUpdateFailed: "Не удалось обновить уведомления. Проверьте подключение и повторите попытку.",
        observation: {
            running: "Выполняется",
            needs_input: "Требуется ответ",
            completed: "Завершено",
            failed: "Ошибка",
            cancelled: "Отменено",
    ...
    browseActivityUnknown: "Неизвестно",
        ...
        browseSearchIncomplete: "Поиск охватил не все сессии; могут быть и другие совпадения. Попробуйте уточнить поисковый запрос.",
...
settingsSession: {
  ...
  welcome: {
    ...
    welcomeQuestionBody: "Codex Top — это центр управления вашими ИИ-агентами для программирования. Email не нужен. Ваш аккаунт — это приватный ключ, сгенерированный на этом устройстве.",

    welcomePrimaryButton: "Впервые здесь — начнём",
    welcomePrimarySubtitle: "Одно касание. Без форм. Ваш ключ хранится здесь.",

    welcomeSecondaryButton: "Войти — я уже пользуюсь Codex Top",
  ...
  sessionGettingStarted: {
      phoneTitle: "Компьютер не подключён",
      phoneSubtitle: "Подключение компьютеров дорабатывается. После подключения здесь появятся сеансы.",
    title: {
  ...
  chatFooter: {
    ...
    directSessionDesktopReady: "Вы можете отправлять текст прямо в эту сессию на компьютере. Будут использованы текущие настройки модели и разрешений настольного приложения.",
    directSessionDesktopUnknown: "Не удалось подтвердить, может ли эта сессия на компьютере принимать текст. Проверьте подключение и повторите попытку.",
    directSessionDesktopUnavailable: "Отправка текста в эту сессию на компьютере недоступна. Проверьте сессию и подключение и повторите попытку.",
    directSessionDesktopDisconnected: "Сессия на компьютере не подключена. Откройте её в настольном приложении и повторите попытку.",
    directSessionDesktopUnsupported: "Это настольное приложение или платформа не поддерживает отправку текста отсюда. Проверьте наличие совместимой версии приложения.",
    directSessionDraftKept: "Сообщение не отправлено. Черновик сохранён.",
  ...
  settingsLanguage: {
        saveFailed: "Не удалось сохранить настройку языка. Повторите попытку.",
