...
const loginContinuation = vi.hoisted(() => ({ authenticated: false, params: {} as Record<string, string>, replace: vi.fn() }));
vi.mock('expo-router', async () => {
    const { createExpoRouterMock } = await import('@/dev/testkit/mocks/router');
    return createExpoRouterMock({ params: () => loginContinuation.params, router: { replace: loginContinuation.replace } }).module;
});
vi.mock('@/sync/domains/server/serverRuntime', () => ({ getActiveServerSnapshot: () => ({ serverId: 'server-b', serverUrl: 'https://b.example.test' }) }));
vi.mock('@/auth/flows/getToken', () => ({ authGetToken: async () => 'synthetic-token' }));
vi.mock('@/platform/cryptoRandom', () => ({ getRandomBytesAsync: async () => new Uint8Array(32) }));
...
vi.mock('@/auth/context/AuthContext', () => ({
    useAuth: () => ({
        isAuthenticated: loginContinuation.authenticated,
        credentials: null,
        login: vi.fn(async () => { loginContinuation.authenticated = true; }),
        logout: vi.fn(async () => {}),
    }),
}));
...
describe('/ (welcome) signup methods', () => {
    beforeEach(() => {
        loginContinuation.authenticated = false;
        loginContinuation.params = {};
        loginContinuation.replace.mockClear();
    ...
    it('匿名创建账号后保留未登录通知的会话来源', async () => {
        loginContinuation.params = { id: 'notification-session', serverId: 'server-b', authReturn: '1' };
        getServerFeaturesSnapshotMock.mockResolvedValue({ status: 'ready', features: createWelcomeFeaturesResponse({ signupMethods: [{ id: 'anonymous', enabled: true }], requiredProviders: [] }) });
        const screen = await renderWelcomeScreen();
        await waitForWelcomeTestId(screen, 'welcome-primary-start');
        await screen.pressByTestIdAsync('welcome-primary-start');
        expect(loginContinuation.authenticated).toBe(true);
        const { default: Home } = await import('@/app/(app)/index');
        await act(async () => { screen.tree.update(<Home />); });
        await flushHookEffects();
        expect(loginContinuation.replace).toHaveBeenCalledWith('/session/notification-session?serverId=server-b');
    });
});
