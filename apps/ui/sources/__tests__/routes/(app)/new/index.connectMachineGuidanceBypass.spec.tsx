...
vi.mock('react-native', async () => {
    ...
    return createReactNativeWebMock({ Platform: { get OS() { return routeState.platformOS; }, select: (values: Record<string, unknown>) => values[routeState.platformOS] ?? values.default }, useWindowDimensions: () => ({ width: 390, height: 800 }) });
});
...
describe('/new connect-machine guidance bypass', () => {
    it('keeps Android blocked without a machine while showing compact phone guidance', async () => {
        routeState.platformOS = 'android';
        routeState.params = {};
        ...
        try {
            ...
            expect(screen.findAllByTestId('guidance:phone')).toHaveLength(1);
            expect(screen.findAllByTestId('new-session-inner')).toHaveLength(0);
        } finally { routeState.platformOS = 'web'; }
    });
