...
vi.mock('@/components/account/ProviderIdentityItems', () => ({
    ProviderIdentityItems: 'ProviderIdentityItems',
}));
...
describe('Settings → Account (grouping)', () => {
    ...
    it('hides "Add your phone" on phone-sized web', async () => {
        ...
        expect(screen.findAllByType('Item' as any).some((row) => row.props.title === 'settingsAccount.linkNewDevice')).toBe(false);
        expect(screen.findAllByType('ProviderIdentityItems' as any)).toHaveLength(0);
        expect(screen.findByTestId('settings-account-secret-key-item')).toBeNull();
        expect(screen.findByTestId('settings-account-phone-identity')).not.toBeNull();
        expect(screen.findByTestId('settings-account-phone-name')?.props.detail).toBe('test-account');
        expect(screen.findByTestId('settings-account-secret-key-value')).toBeNull();
        expect(screen.findByTestId('settings-account-logout')).not.toBeNull();
    });
