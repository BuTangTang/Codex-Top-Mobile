...
const recoveryState = vi.hoisted(() => ({ drafts: null as readonly AttachmentDraft[] | null }));
...
vi.mock('@/utils/sessions/tempDataStore', () => ({
    getTempData: (id: string) => id === 'synthetic-recovery' ? { attachmentDrafts: recoveryState.drafts } : null,
}));
...
describe('session route index', () => {
    ...
    it('keeps navigation available during a cold load when the owning server is offline', async () => {
    ...
    it('does not apply the active server offline state to a different route server', async () => {
        endpointConnectivityStatus = 'offline';
        routeParams.value = { id: 'session-1', serverId: 'other-server' };
        hydrateSessionForRouteSpy.mockReturnValue({ kind: 'loading', sessionId: 'session-1', serverId: 'other-server', reason: 'cold' });
        const Route = await import('@/app/(app)/session/[id]');
        const screen = await renderScreen(React.createElement(Route.default));
        expect(screen.findAllByType('ActivitySpinner')).toHaveLength(1);
    });
    ...
    it('renders the session cockpit shell on mobile web when cockpit mode is enabled', async () => {
        platformState.os = 'web';
        deviceType = 'phone';
        mobileWorkspaceExperience = 'cockpit';
    ...
    it('keeps native phone notifications and recovered drafts on the original classic session route', async () => {
        platformState.os = 'android';
        deviceType = 'phone';
        mobileWorkspaceExperience = 'cockpit';
        lastMobileSurfaceBySessionId = { 'server-b:session-1': 'git' };
        const draft: AttachmentDraft = { id: 'synthetic-draft', status: 'pending', source: { kind: 'memory', name: 'sample.txt', bytes: new Uint8Array([65]) } };
        recoveryState.drafts = [draft];
        routeParams.value = { id: 'session-1', serverId: 'server-b', jumpSeq: '42', recoveryDataId: 'synthetic-recovery' };
        const Route = await import('@/app/(app)/session/[id]');
        const screen = await renderScreen(React.createElement(Route.default));
        expect(screen.findAllByType('SessionCockpitShell')).toHaveLength(0);
        const sessionView = screen.findByType('SessionView' as never);
        ...
        expect(sessionView.props.initialAttachmentDrafts).toEqual([draft]);
        expect(sessionView.props.routeAnchorOverride).toBe(true);
    });
});
