...
import { isRecoveredHistoryTranscriptObservationProvenance, type DirectTranscriptHistoryAvailability } from '@happier-dev/protocol';
...
export type SessionMessages = {
    /** 仅在当前内存会话中保留；缺失表示旧来源尚未报告可读性。 */
    directHistoryAvailability?: DirectTranscriptHistoryAvailability;
...
export type MessagesDomain = {
    setDirectSessionHistoryAvailability: (sessionId: string, availability: DirectTranscriptHistoryAvailability | undefined) => void;
...
function coerceSessionMessages(input: unknown): SessionMessages {
    ...
    return {
        ...
        directHistoryAvailability: raw?.directHistoryAvailability,
        messagesById,
...
export function createMessagesDomain<S extends MessagesDomain & MessagesDomainDependencies>({
    ...
    return {
        ...
        setDirectSessionHistoryAvailability: (sessionId, availability) => set((state) => {
            const existing = state.sessionMessages[sessionId];
            if (existing?.directHistoryAvailability === availability) return state;
            return { ...state, sessionMessages: { ...state.sessionMessages, [sessionId]: {
                ...(existing ?? createEmptySessionMessages()), directHistoryAvailability: availability,
            } } };
        }),
