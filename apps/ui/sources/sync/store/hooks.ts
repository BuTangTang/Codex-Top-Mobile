...
/** 只订阅单个会话的历史可用状态，不随正文流式更新重建提示。 */
export function useSessionDirectHistoryAvailability(sessionId: string) {
  return getStorage()((state) => {
    if (readDirectSessionLink(state.sessions[sessionId]?.metadata)?.providerId !== 'codex') return undefined;
    return state.sessionMessages[sessionId]?.directHistoryAvailability ?? 'unknown';
  });
}
