...
/**
 * 在指定机器发起远程会话；保留原有兼容性校验及失败结果，仅使用当前产品显示名。
 */
    ...
    try {
        ...
        if (
            ...
            return {
                ...
                errorCode: SPAWN_SESSION_ERROR_CODES.INVALID_REQUEST,
                errorMessage:
                    '请更新或重新连接命令行组件，以继续此会话。继续已有'
                    + '会话需要此电脑运行 Codex Top CLI 0.2.10-dev.76 或更新版本'
                    + `（检测到 ${versionLabel}）。`,
            };
        }
        ...
        if (
            ...
            return {
                ...
                errorCode: SPAWN_SESSION_ERROR_CODES.INVALID_REQUEST,
                errorMessage:
                    '所选后端要求此电脑运行兼容的 0.1.0-dev 构建，或 Codex Top CLI v0.2.0 ' +
                    `或更新版本（检测到 ${versionLabel}）。`,
            };
        }
