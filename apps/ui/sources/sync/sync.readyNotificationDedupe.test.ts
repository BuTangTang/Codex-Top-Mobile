...
describe('Sync ready notification dedupe', () => {
    ...
    it.each([null, 'codex'] as const)('preserves ordinary runner ready notifications and dedupe with flavor %s', async (flavor) => {
        const { sync } = await import('./sync');
        const syncForTest = sync as unknown as SyncReadyNotificationTestAccess;
        const session = createSession('s1');
        if (flavor) session.metadata = { path: '', host: '', flavor };
