...
function createDirectSession(sessionId: string, providerId: 'codex' | 'claude' = 'codex'): Session & { metadata: NonNullable<Session['metadata']> } {
    const now = Date.now();
    return {
        ...
        metadata: {
            ...
            directSessionV1: {
                ...
                source: providerId === 'codex' ? { kind: 'codexHome', home: 'user' } : { kind: 'claudeConfig' },
            },
        },
    };
}
...
describe('sync.fetchMessages server-scoped known-session checks', () => {
    ...
    it.each(['preview_only', 'unavailable'] as const)('preserves accepted history through %s reads and clears the warning only after a readable snapshot', async (historyAvailability) => {
        const sessionId = `history_availability_${historyAvailability}`;
        storage.getState().applySessions([createDirectSession(sessionId, 'codex')]);
        const acceptedPage = {
            ok: true, historyAvailability: 'available',
            items: [{ id: 'history-anchor', createdAtMs: 1, raw: { role: 'user', content: { type: 'text', text: 'accepted history' } } }],
            nextCursor: 'accepted-older', tailCursor: 'accepted-tail', hasMore: true,
        };
        machineDirectSessionTranscriptPageMock.mockResolvedValueOnce(acceptedPage);
        const { sync } = await import('./sync');
        ...
        await internals.fetchMessages(sessionId);
        const acceptedIds = storage.getState().sessionMessages[sessionId]!.messageIdsOldestFirst;
        const unavailablePage = { ok: true, historyAvailability, items: [], nextCursor: null, tailCursor: null, hasMore: false, truncated: false };
        machineDirectSessionTranscriptReadAfterMock.mockResolvedValue(unavailablePage);
        machineDirectSessionTranscriptPageMock.mockResolvedValue(unavailablePage);

        await internals.fetchMessages(sessionId);
        await sync.loadOlderMessages(sessionId);
        expect(storage.getState().sessionMessages[sessionId]).toMatchObject({ directHistoryAvailability: historyAvailability });
        expect(storage.getState().sessionMessages[sessionId]!.messageIdsOldestFirst).toEqual(acceptedIds);
        ...
        // 增量重新可读不能单独证明旧历史已恢复；成功快照才解除提示。
        machineDirectSessionTranscriptReadAfterMock.mockResolvedValue({ ...unavailablePage, historyAvailability: 'available', nextCursor: 'unaccepted-tail' });
        await internals.fetchMessages(sessionId);
        expect(storage.getState().sessionMessages[sessionId]).toMatchObject({ directHistoryAvailability: historyAvailability });
        machineDirectSessionTranscriptPageMock.mockResolvedValue({ ...acceptedPage, nextCursor: null, hasMore: false });
        await internals.fetchMessages(sessionId);
        expect(storage.getState().sessionMessages[sessionId]).toMatchObject({ directHistoryAvailability: 'available' });
        expect(storage.getState().sessionMessages[sessionId]!.messageIdsOldestFirst).toEqual(acceptedIds);
    });

    it.each(['preview_only', 'unavailable', undefined] as const)('publishes first-read history state %s without inventing accepted cursors', async (historyAvailability) => {
        const sessionId = `history_initial_${historyAvailability ?? 'unknown'}`;
        storage.getState().applySessions([createDirectSession(sessionId)]);
        machineDirectSessionTranscriptPageMock.mockResolvedValueOnce({
            ok: true, historyAvailability,
            items: historyAvailability === 'preview_only' ? [{ id: 'preview', createdAtMs: 1, raw: { role: 'user', content: { type: 'text', text: 'preview' } } }] : [],
            nextCursor: null, tailCursor: 'preview-tail', hasMore: false,
        });
        const { sync } = await import('./sync');
        const internals = sync as unknown as { fetchMessages: (id: string) => Promise<void>; getDirectSessionTailCursor: (id: string) => string | null };
        await internals.fetchMessages(sessionId);
        expect(storage.getState().sessionMessages[sessionId]?.directHistoryAvailability).toBe(historyAvailability);
        expect(storage.getState().sessionMessages[sessionId]?.isLoaded).toBe(true);
        expect(internals.getDirectSessionTailCursor(sessionId)).toBe(historyAvailability ? null : 'preview-tail');
        expect(storage.getState().sessionMessages[sessionId]?.messageIdsOldestFirst).toHaveLength(historyAvailability === 'preview_only' ? 1 : 0);
    });

    it('rechecks a snapshot when an old unknown source starts reporting readable tail history', async () => {
        const sessionId = 'history_old_source_upgrade';
        storage.getState().applySessions([createDirectSession(sessionId)]);
        const page = { ok: true, items: [], nextCursor: null, tailCursor: 'accepted-tail', hasMore: false };
        machineDirectSessionTranscriptPageMock.mockResolvedValueOnce(page);
        const { sync } = await import('./sync');
        const internals = sync as unknown as { fetchMessages: (id: string) => Promise<void> };
        await internals.fetchMessages(sessionId);
        expect(storage.getState().sessionMessages[sessionId]?.directHistoryAvailability).toBeUndefined();
        machineDirectSessionTranscriptReadAfterMock.mockResolvedValueOnce({ ...page, nextCursor: 'readable-tail', truncated: false, historyAvailability: 'available' });
        machineDirectSessionTranscriptPageMock.mockResolvedValueOnce({ ...page, historyAvailability: 'available' });
        await internals.fetchMessages(sessionId);
        expect(storage.getState().sessionMessages[sessionId]?.directHistoryAvailability).toBe('available');
        expect(machineDirectSessionTranscriptPageMock).toHaveBeenCalledTimes(2);
    });

    it('ignores a held unavailable history page after the linked source is replaced', async () => {
        const sessionId = 'history_availability_changed_source';
        const session = createDirectSession(sessionId);
        storage.getState().applySessions([session]);
        const page = { ok: true, historyAvailability: 'available', items: [], nextCursor: 'older', tailCursor: 'tail', hasMore: true };
        machineDirectSessionTranscriptPageMock.mockResolvedValueOnce(page);
        const { sync } = await import('./sync');
        const internals = sync as unknown as { fetchMessages: (id: string) => Promise<void> };
        await internals.fetchMessages(sessionId);
        ...
        const loading = sync.loadOlderMessages(sessionId);
        storage.getState().applySessions([{ ...session, metadata: { ...session.metadata,
            directSessionV1: { ...session.metadata.directSessionV1!, remoteSessionId: 'new-source' },
        } }]);
        machineDirectSessionTranscriptPageMock.mockResolvedValueOnce({ ...page, nextCursor: null, hasMore: false });
        await internals.fetchMessages(sessionId);
        held.resolve({ ...page, historyAvailability: 'unavailable', hasMore: false, nextCursor: null });
        await loading;
        expect(storage.getState().sessionMessages[sessionId]?.directHistoryAvailability).toBe('available');
    });
    ...
    it.each(['codex', 'claude'] as const)('routes direct-session replayed ready notifications by linked provider (%s)', async (providerId) => {
        const sessionId = `direct_session_push_ready_notification_${providerId}`;
        storage.getState().applySessions([createDirectSession(sessionId, providerId)]);

        const { sync } = await import('./sync');
        const syncForTest = sync as unknown as {
            handleEphemeralUpdate: (update: unknown) => void;
        };
        ...
        const update = {
            ...
            items: [
                {
                    id: 'direct-historical-reply-1',
                    createdAtMs: 1,
                    raw: {
                        role: 'agent',
                        content: {
                            type: 'codex',
                            data: { type: 'message', message: 'historical direct reply' },
                        },
                    },
                },
                {
        ...
        syncForTest.handleEphemeralUpdate(update);
        await vi.waitFor(() => {
            expect(Object.values(storage.getState().sessionMessages[sessionId]?.messagesById ?? {}))
                .toContainEqual(expect.objectContaining({ kind: 'agent-text', text: 'historical direct reply' }));
        });
        if (providerId === 'codex') {
    ...
    it.each(['codex', 'claude'] as const)('routes direct-session intermediate text notifications by linked provider (%s)', async (providerId) => {
        const sessionId = `direct_session_push_agent_reply_notification_${providerId}`;
        storage.getState().applySessions([createDirectSession(sessionId, providerId)]);
        ...
        syncForTest.handleEphemeralUpdate({
            type: 'direct-session-transcript-delta',
            sessionId,
            items: [
                {
                    ...
                    raw: {
                        ...
                        content: {
                            ...
                            data: {
                                type: 'message',
                                message: 'Still working on the next step',
                            },
                        },
                    },
                },
        ...
        await vi.waitFor(() => {
            expect(Object.values(storage.getState().sessionMessages[sessionId]?.messagesById ?? {}))
                .toContainEqual(expect.objectContaining({ kind: 'agent-text', text: 'Still working on the next step' }));
        });
        ...
        if (providerId === 'codex') {
            ...
            expect(notifyActivityReadyMock).toHaveBeenCalledWith(sessionId, [
                expect.objectContaining({ kind: 'agent-text', text: 'Still working on the next step' }),
            ]);
        }
    });
