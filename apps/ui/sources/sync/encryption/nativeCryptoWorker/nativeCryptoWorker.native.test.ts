...
describe('createNativeCryptoWorker native', () => {
    ...
    it.each([' 密码 test ', ' 🔐 密码\0tail ', '\0'])('preserves UTF-8 input and all native PBKDF2 output bytes for %j', async (password) => {
        const passwordUtf8 = new TextEncoder().encode(password);
        const salt = new Uint8Array(32).fill(7);
        const expected = pbkdf2Sync(passwordUtf8, salt, 220000, 64, 'sha512');
        nativeModuleMock.requireNativeModule.mockReturnValueOnce({
            /** 原生边界替身只核对传输编码，算法标准结果独立由 Node 生成。 */
            derivePasswordMasterV1: async (encodedPassword: string, encodedSalt: string) => {
                expect(Buffer.from(encodedPassword, 'base64')).toEqual(Buffer.from(passwordUtf8));
                expect(Buffer.from(encodedSalt, 'base64')).toEqual(Buffer.from(salt));
                return expected.toString('base64');
            },
        });
        const { deriveNativePasswordMasterKey } = await import('./nativeCryptoWorker.native');
        expect(Buffer.from(await deriveNativePasswordMasterKey({ passwordUtf8, salt, iterations: 220000, length: 64 }))).toEqual(expected);
    });

    it('fails explicitly when a native password KDF is missing or has an invalid output', async () => {
        nativeModuleMock.requireNativeModule.mockReturnValueOnce({});
        const { deriveNativePasswordMasterKey } = await import('./nativeCryptoWorker.native');
        const input = { passwordUtf8: new TextEncoder().encode('synthetic'), salt: new Uint8Array(32), iterations: 220000 as const, length: 64 as const };
        await expect(deriveNativePasswordMasterKey(input)).rejects.toThrow('Native password derivation unavailable');
        vi.resetModules();
        nativeModuleMock.requireNativeModule.mockReturnValueOnce({ derivePasswordMasterV1: async () => Buffer.alloc(32).toString('base64') });
        const reloaded = await import('./nativeCryptoWorker.native');
        await expect(reloaded.deriveNativePasswordMasterKey(input)).rejects.toThrow('Invalid native password derivation result');
    });
