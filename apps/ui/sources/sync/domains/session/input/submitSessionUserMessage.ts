...
/** 复用发送回执、草稿交接和错误处理，外部拥有者失败时也不切换到队列。 */
async function directSend(
    ...
    try {
        ...
        const sendOptions = {
            ...
            ...(opts.directSessionExternalControl ? { directSessionExternalControl: true as const } : {}),
...
/** 按本次能力探测选路；桌面文本路径不执行普通队列的唤醒与接管策略。 */
export async function submitSessionUserMessage(
    port: SessionSubmitPort,
    opts: SubmitSessionUserMessageOptions,
): Promise<SubmitSessionUserMessageResult> {
    if (opts.directSessionExternalControl) {
        // 桌面首版只支持普通文本；保留用户显式意图，拒绝不支持的队列或配置操作。
        if (opts.explicitMode === 'server_pending' || opts.explicitMode === 'interrupt'
            || opts.forceImmediate || opts.requestedAction || opts.applyConfigAndSteer
            || opts.steerWithoutConfig || opts.existingDurablePendingMessage) {
            return {
                type: 'rejected',
                persistence: 'none',
                wake: { attempted: false, state: 'not_needed' },
                errorCode: 'unsupported_input',
                errorMessage: 'This desktop session supports plain text with its current desktop settings. Queue, interrupt, and configuration changes are unavailable.',
            };
        }
        return directSend(port, opts, 'selected_direct');
    }
