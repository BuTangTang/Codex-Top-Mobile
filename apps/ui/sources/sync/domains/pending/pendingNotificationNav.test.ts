...
describe('unauthenticated notification target', () => {
    it('retains one restricted target and never assigns it to a different login', async () => {
        const mod = await import('./pendingNotificationNav');
        const { registerStorageStateReader } = await import('@/sync/domains/state/storageStateReaderBridge');
        registerStorageStateReader(() => ({ profileScope: null } as unknown as StorageState));
        const pending = { serverUrl: 'https://login-target.test', route: '/session/original', target: { accountId: 'owner', machineId: 'machine-a', notificationIdentity: 'a'.repeat(64) } };
        mod.setPendingNotificationNav(pending);
        expect(mod.getPendingNotificationNav()).toBeNull();
        await activateServerAccount('https://login-target.test', 'different');
        expect(mod.getPendingNotificationNav()).toBeNull();
        await activateServerAccount('https://other-target.test', 'owner');
        expect(mod.getPendingNotificationNav()).toBeNull();
        await activateServerAccount('https://login-target.test', 'owner');
        expect(mod.getPendingNotificationNav()).toEqual(pending);
        mod.clearPendingNotificationNav();
        expect(mod.getPendingNotificationNav()).toBeNull();
    });
});
