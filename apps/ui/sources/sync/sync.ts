...
import {
    ...
    type DirectTranscriptHistoryAvailability,
...
class Sync {
    ...
    private flushPendingSettingsForCurrentScopeNow(): void {
        ...
        // 立即保存接管原定时任务；失败保留 dirty 和 pending，由调用方展示错误并重试。
    ...
    /** 在重启前同步保存设置意图，防止确认期间切账户后写入另一账户。 */
    applySettingsBeforeReload(delta: Partial<Settings>, expectedScope: AccountSettingsScope | null): void {
        if (!expectedScope
            || !areAccountSettingsScopesEqual(expectedScope, this.pendingSettingsScope)
            || !areAccountSettingsScopesEqual(expectedScope, storage.getState().settingsScope)
            || expectedScope.serverId !== getActiveServerSnapshot().serverId) {
            throw new Error('Account settings scope changed before saving');
        }
        this.applySettings(delta, { source: 'ui' });
    ...
    async sendMessage(
        ...
        try {
            const externalControlRequested = options?.directSessionExternalControl === true;
            const externalLink = externalControlRequested ? readDirectSessionLink(session.metadata) : null;
            if (externalControlRequested && !externalLink) {
                throw new Error('Direct session link is unavailable');
            }
            if (externalControlRequested && options?.profileId) {
                throw new Error('unsupported_input: external desktop text cannot apply a profile');
            }
            // 在任何异步处理和投影回调前固定本次关联目标，不能随后随活动服务器改道。
            const externalTarget = externalLink ? {
                machineId: externalLink.machineId,
                serverId: this.getDirectSessionServerScope(sessionId),
            } : null;
            // 缺失归属必须在创建投影前拒绝，不能把 undefined 留给稍后的活动服务器解释。
            if (externalTarget && !externalTarget.serverId) {
                throw new Error('Direct session server scope is unavailable');
            }
            ...
            const content = buildOutgoingUserTextRecord({
                ...
                sentFrom,
                displayText,
                // 桌面保留执行设置；不注入 provider 附加语义，显式元数据交由接收端校验。
                agentId: externalTarget ? null : agentId,
                modelMode,
                permissionMode,
            ...
            const messagePayload = externalTarget ? undefined :
            ...
            if (externalTarget) {
                ...
                let issued = false;
                let ack: SessionUserMessageSendResponse;
                try {
                    ack = await machineDirectSessionSend({
                        machineId: externalTarget.machineId,
                        sessionId,
                        text,
                        localId,
                        meta: content.meta ?? {},
                ...
                // 原生记录继续由现有 Direct transcript 跟随读取，不伪造已提交的消息或发布权限设置。
                return {
                    localId,
                    persistence: 'provider_direct' as const,
                    ...(ack.providerAcceptancePending === true ? { providerAcceptancePending: true } : {}),
                };
            }
    ...
    private fetchMessages = async (sessionId: string) => {
      ...
      /** 历史读取失败不能推进正文边界；仅首次预览可作为非权威内容展示。 */
      private acceptDirectHistoryRead(
          sessionId: string,
          link: DirectSessionLink,
          page: Readonly<{ historyAvailability?: DirectTranscriptHistoryAvailability; items: readonly DirectTranscriptRawMessageV1[] }>,
          allowInitialPreview = false,
      ): boolean {
          ...
          if (allowInitialPreview && page.historyAvailability === 'preview_only'
              ...
              this.applyMessages(sessionId, normalizeDirectTranscriptMessages(page.items), { notifyVoice: false, notifyActivity: false });
          }
      ...
      private async fetchDirectSessionMessages(
          ...
          if (!this.acceptDirectHistoryRead(sessionId, directSessionLink, page, true)) return;
          ...
          if (typeof tailCursor !== 'string' || tailCursor.trim().length === 0) {
              ...
              if (!this.acceptDirectHistoryRead(sessionId, directSessionLink, tail)) return;
      ...
      private async catchUpDirectSessionMessages(
          ...
          const probeAndApply = async (): Promise<number> => {
              ...
              const historyAvailability = storage.getState().sessionMessages[sessionId]?.directHistoryAvailability;
              const needsHistoryRecovery = directSessionLink.providerId === 'codex'
                  && (historyAvailability === 'preview_only' || historyAvailability === 'unavailable');
              ...
              if (!this.acceptDirectHistoryRead(sessionId, directSessionLink, tail)) return 0;
      ...
      private async applyDirectSessionTranscriptItems(
          ...
          if (normalizedMessages.length > 0) {
              // Codex Direct 完成通知由显式生命周期生产者负责，正文及重放的 ready 不触发本地通知。
              const notifyActivity = directSessionLink.providerId !== 'codex';
              const applied = this.applyMessages(sessionId, normalizedMessages, { notifyVoice: false, notifyActivity });
              if (notifyActivity && !applied.hasReadyEvent) {
                  const sessionMessages = storage.getState().sessionMessages[sessionId];
          ...
          if (params.scope === 'main') {
              ...
              if (directSessionLink) {
                  ...
                  try {
                      ...
                      if (!this.acceptDirectHistoryRead(params.sessionId, directSessionLink, page)) {
                          return { loaded: 0, hasMore: knownHasMore ?? true, status: 'not_ready' };
                      }
