...
/** 注册使用捕获的账号凭据时不重新读取其他登录；信号只限制本次网络等待。 */
export async function registerPushToken(
    credentials: AuthCredentials,
    token: string,
    opts: Readonly<{ serverId?: string; apiEndpoint?: string; clientServerUrl?: string; retry?: 'default' | 'none'; useProvidedCredentials?: boolean; signal?: AbortSignal }> = {},
): Promise<void> {
    ...
    const run = async () => {
        const endpointCredentials = (() => {
            if (!API_ENDPOINT || opts.useProvidedCredentials) return null;
...
/** 只删除指定服务中本账号的设备 token，不扩大为其他账号或其他端退出。 */
export async function deletePushToken(
    credentials: AuthCredentials,
    token: string,
    opts: Readonly<{ apiEndpoint?: string; signal?: AbortSignal }> = {},
): Promise<void> {
    const API_ENDPOINT = (opts.apiEndpoint ?? '').trim().replace(/\/+$/, '');
