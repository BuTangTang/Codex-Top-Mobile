...
/** 可选固定登录服务器，避免异步保存跨越服务器切换后写入另一作用域。 */
async function getAuthAutoRedirectSuppressedUntilKey(scope?: AuthServerScope): Promise<string> {
    const expected = scope?.expectedActiveServerSnapshot;
    return (await getServerScopedKeys(AUTH_AUTO_REDIRECT_SUPPRESSED_UNTIL_KEY, expected?.serverUrl, expected ? { serverId: expected.serverId } : {})).primary;
}
...
export const TokenStorage = {
    ...
    /** 在原作用域清除自动跳转抑制；守卫失效后不继续写其他服务器。 */
    async setAuthAutoRedirectSuppressedUntil(value: number, scope?: AuthServerScope): Promise<boolean> {
        assertAuthServerScopeCurrent(scope);
        const key = await getAuthAutoRedirectSuppressedUntilKey(scope);
        assertAuthServerScopeCurrent(scope);
        const globalKey = getAuthAutoRedirectSuppressedUntilGlobalKey();
    ...
    /** 复用原凭据 owner，登录可固定服务器并在每个异步写入后核对仍有效。 */
    async setCredentials(credentials: AuthCredentials, scope?: AuthServerScope): Promise<boolean> {
        assertAuthServerScopeCurrent(scope);
        const expected = scope?.expectedActiveServerSnapshot;
        const keys = await getAuthKeys(expected?.serverUrl, expected ? { serverId: expected.serverId } : {});
        assertAuthServerScopeCurrent(scope);
        ...
        try {
            ...
            await TokenStorage.setAuthAutoRedirectSuppressedUntil(0, scope);
            assertAuthServerScopeCurrent(scope);
            ...
            // 凭据或后续迁移写盘期间退出、切服时都撤销本次 token，保留较新的登录。
            if (expected) await TokenStorage.invalidateCredentialsTokenForServerUrl(expected.serverUrl, credentials.token, { serverId: expected.serverId });
            throw error;
        }
    },
