...
/** 固定一次扫码等待的请求来源，迟到响应不能给切换后的服务器写身份或返回凭据。 */
export async function authQRWait(keypair: QRAuthKeyPair, onProgress?: (dots: number) => void, shouldCancel?: () => boolean): Promise<AuthCredentials | null> {
    let dots = 0;
    const source = { ...getActiveServerSnapshot() };
    /** 同一服务器的新代次也使旧请求失效；取消检查覆盖响应体解析期间。 */
    const isCurrent = (): boolean => {
        const current = getActiveServerSnapshot();
        return current.serverId === source.serverId && current.serverUrl === source.serverUrl
    ...
    while (true) {
        ...
        try {
            ...
            if (!isCurrent() || shouldCancel?.()) return null;
            ...
            // serverFetch 检查响应头返回时的代次；这里补上异步读取响应体后的同源检查。
            if (!isCurrent() || shouldCancel?.()) return null;
            ...
            if (data.state === 'authorized') {
                ...
                if (data.serverIdentityId) {
                    setServerProfileIdentityForUrl(source.serverUrl, data.serverIdentityId);
                }
