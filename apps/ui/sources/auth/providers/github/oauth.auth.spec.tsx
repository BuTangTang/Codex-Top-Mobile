...
describe('OAuth迟到导航', () => {
    it('新通知使callback失焦后，不消费旧finalize结果', async () => {
        setPendingExternalAuthState({ provider: 'github', secret: OAUTH_SECRET, returnTo: '/?id=old&serverId=server-a&authReturn=1' });
        localSearchParamsMock.mockReturnValue({ provider: 'github', flow: 'auth', pending: 'old' });
        let finish!: (value: FetchResult) => void;
        stubFetch(async (url) => {
            const health = await handleHealthCheck(url);
            if (health) return health;
        ...
        await runWithOAuthScreen(async (tree) => {
            ...
            loginSpy.mockClear();
            await act(async () => { finish({ ok: true, body: { success: true, token: 'test' } }); });
            await flushOAuthEffects();
            expect(loginSpy).not.toHaveBeenCalled();
