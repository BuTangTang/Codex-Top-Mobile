...
export default function OAuthProviderReturn() {
    ...
    const routeKey = JSON.stringify([params.provider, params.flow, params.pending, params.login]);
    const lifecycleRef = React.useRef({ active: focused, routeKey });
    // 失焦或新回调使旧组件操作永久失效；重新进入是新的组件生命周期，不恢复旧异步导航。
    if (!focused || lifecycleRef.current.routeKey !== routeKey) lifecycleRef.current.active = false;
    if (focused && !lifecycleRef.current.active) lifecycleRef.current = { active: true, routeKey };
    const lifecycle = lifecycleRef.current;
    React.useEffect(() => () => { lifecycle.active = false; }, [lifecycle]);
    const router = React.useMemo(() => ({
        ...navigationRouter,
        replace: (href: Parameters<typeof navigationRouter.replace>[0]) => {
            if (lifecycle.active) navigationRouter.replace(href);
        },
    }), [navigationRouter, lifecycle]);
    const auth = useAuth();
    ...
    const finalizeAuth = React.useCallback((params: { mode: 'plain' | 'e2ee' }) => {
        ...
        const sourceServerId = getActiveServerSnapshot().serverId;
        ...
        fireAndForget((async () => {
            ...
            try {
                ...
                // 网络返回时重新验证焦点与实际服务器，尚未登录的旧请求到此停止。
                if (!lifecycle.active || !areServerProfileIdentifiersEquivalent(sourceServerId, getActiveServerSnapshot().serverId)) return;
