...
function NewSessionContent(props: Readonly<{
    ...
    // 只改变手机阻塞提示的展示，仍由原模型决定是否允许进入新建流程。
    const isAndroidPhone = Platform.OS === 'android' && isMobileLayoutWidth(width);

    if (props.allowBlockingGuidance && shouldBlock) {
        return (
            <>
                <NewSessionWebCloseFallback />
                <SessionGettingStartedGuidance variant={isAndroidPhone ? "phone" : "newSessionBlocking"} />
            </>
        );
    }
