...
export default React.memo(() => {
    ...
    // 离线时让会话壳提供返回与恢复入口，不让尚未结束的读取遮住整个页面。
    const owningServerId = routeHydrationState.serverId ?? routeScope.serverId ?? activeServerSnapshot.serverId;
    const owningServerOffline = endpointConnectivity.status === 'offline'
        && (owningServerId === activeServerSnapshot.serverId
            || areServerProfileIdentifiersEquivalent(owningServerId ?? '', activeServerSnapshot.serverId ?? ''));
    if (routeHydrationState.kind === 'loading' && !sessionCached && !authRecoveryActive && !owningServerOffline) {
        return (
