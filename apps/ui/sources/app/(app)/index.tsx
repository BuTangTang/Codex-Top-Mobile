import { PhonePasswordLoginView } from '@/components/account/auth/PhonePasswordLoginView';
import { useDeviceType } from '@/utils/platform/responsive';
import { resolveSessionAuthReturnTo, buildAuthRestoreRoute } from '@/auth/routing/authRouting';
...
/** 手机未登录入口使用账号密码；既有桌面恢复与已登录会话入口保持原 owner。 */
export default function Home() {
    const auth = useAuth();
    const deviceType = useDeviceType();
    if (!auth.isAuthenticated) {
        if (Platform.OS !== 'web' && deviceType === 'phone') return <PhonePasswordLoginView />;
...
function Authenticated() {
    ...
    React.useEffect(() => {
        ...
        // 登录返回只能消费当前服务器的授权，普通跨服 deep link 保留既有行为。
        if (params.authReturn === '1' && !resolveSessionAuthReturnTo(params, getActiveServerSnapshot().serverId)) {
            router.replace('/');
            return;
        }
        ...
        router.replace(href);
    }, [jumpChildId, messageId, router, serverId, sessionId, params.authReturn]);
...
/** 优先恢复既有桌面 setup，否则使用当前服务器的会话登录返回目标。 */
function resolveAuthReturnToRoute(params: Readonly<Record<string, unknown>>): string {
    const pendingSetupIntent = getPendingSetupIntent();
    const resumesRemoteMachineSetup = pendingSetupIntent?.phase === 'awaiting_auth'
        && pendingSetupIntent.branch === 'remoteMachine';
    return resumesRemoteMachineSetup && isTauriDesktop() ? '/setup' : resolveSessionAuthReturnTo(params, getActiveServerSnapshot().serverId) ?? '/';
}
...
function NotAuthenticated() {
    ...
    const createAccountViaProvider = async (providerId: string) => {
        try {
            ...
            await TokenStorage.setPendingExternalAuth({
                ...
                proof,
                secret,
                returnTo: resolveAuthReturnToRoute(params),
                ...(serverUrl ? { serverUrl } : {}),
            });
    ...
    const handleRestore = () => {
        ...
        router.push(buildAuthRestoreRoute('/restore', params, getActiveServerSnapshot().serverId));
    };
