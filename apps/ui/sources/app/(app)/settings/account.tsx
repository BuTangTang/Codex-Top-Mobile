...
import { PhoneAccountSummary } from '@/components/settings/account/PhoneAccountSummary';
...
export default React.memo(() => {
    ...
    const legacySecret =
        !isPhoneLayout && auth.credentials && isLegacyAuthCredentials(auth.credentials)
    ...
    React.useEffect(() => {
        if (isPhoneLayout || !encryptionAccountOptOutEnabled) return;
    ...
    }, [auth.credentials?.token, encryptionAccountOptOutEnabled, isPhoneLayout]);
    ...
    /** 用户确认后复用认证 owner 清理当前设备，再返回登录入口。 */
    const handleLogout = async () => {
        const confirmed = await Modal.confirm(
            t('common.logout'),
            isPhoneLayout ? '退出当前设备的登录？其他已登录设备不受影响。' : t('settingsAccount.logoutConfirm'),
    ...
    if (isPhoneLayout) {
        return <PhoneAccountSummary profile={profile} loginName={typeof auth.credentials?.loginName === 'string' ? auth.credentials.loginName : undefined} isAuthenticated={auth.isAuthenticated} onLogout={handleLogout} />;
    }

    const showAddYourPhone = isRunningOnMac() || (Platform.OS === 'web' && !isPhoneSizedWeb);
    const showLinkNewDevice = !isPhoneLayout && !isRunningOnMac() && (Platform.OS !== 'web' || isPhoneSizedWeb);
        ...
        <>
            <ItemList>
                ...
                <ItemGroup title={t('settingsAccount.accountInformation')}>
                    <Item
                        density={isPhoneLayout ? "cozy" : undefined}
                        titleLines={isPhoneLayout ? 0 : undefined}
                ...
                {/* Profile Section */}
                {!isPhoneLayout || displayName ? <ItemGroup title={t('settingsAccount.profile')}>
                        {displayName && (
                            <Item
                                density={isPhoneLayout ? "cozy" : undefined}
                                titleLines={isPhoneLayout ? 0 : undefined}
                        ...
                        {!isPhoneLayout ? <ProviderIdentityItems
                            profile={profile}
                ...
                {!isPhoneLayout && profile.connectedServices && profile.connectedServices.length > 0 && (() => {
                ...
                {/* Analytics Section */}
                {!isPhoneLayout && encryptionAccountOptOutEnabled && (
