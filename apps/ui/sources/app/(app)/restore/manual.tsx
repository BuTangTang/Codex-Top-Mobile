...
import { resolveSessionAuthReturnTo, buildSessionAuthReturnTo } from '@/auth/routing/authRouting';
...
export default function Restore() {
    ...
    /** 只允许仍在当前恢复页面的操作完成导航，失焦后迟到结果不得覆盖新通知。 */
    const handleRestore = async () => {
        const loginServerId = getActiveServerSnapshot().serverId;
        const isCurrent = () => focusLifetime.active;
        ...
        try {
            ...
            if (isCurrent()) router.dismissTo(resolveSessionAuthReturnTo(params, getActiveServerSnapshot().serverId) ?? '/');
