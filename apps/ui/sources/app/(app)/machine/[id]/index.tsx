import { PhoneMachineDetails } from '@/components/settings/machines/PhoneMachineDetails';
...
/** 手机挂载紧凑只读详情，不启动桌面专家设置和进程管理的副作用。 */
export default function MachineDetailScreen() {
    const deviceType = useDeviceType();
    const { id, serverId } = useLocalSearchParams<{ id: string; serverId?: string }>();
    if (Platform.OS !== 'web' && deviceType === 'phone') {
        return <PhoneMachineDetails machineId={id ?? ''} serverId={serverId} />;
    }
    return <DesktopMachineDetailScreen />;
}

/** 保留桌面既有机器管理页面及行为。 */
function DesktopMachineDetailScreen() {
