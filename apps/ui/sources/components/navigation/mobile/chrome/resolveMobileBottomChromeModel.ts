...
/** 在设置父路由之前识别电脑入口，供主导航及底栏状态共用。 */
export function resolveMainAppTab(pathname: string): TabType | null {
    if (pathname === '/') return 'sessions';
    if (pathname === '/settings/machines' || pathname.startsWith('/settings/machines/')) return 'machines';
