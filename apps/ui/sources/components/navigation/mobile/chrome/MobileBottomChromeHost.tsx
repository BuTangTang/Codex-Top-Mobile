...
import { resolveMainAppTab } from './resolveMobileBottomChromeModel';
...
export const MobileBottomChromeHost = React.memo(function MobileBottomChromeHost(props: Readonly<{
    ...
    const params = useGlobalSearchParams<{ mobileSurface?: string | string[]; serverId?: string | string[]; machineId?: string | string[]; projectId?: string | string[]; sourceKey?: string | string[] }>();
    const auth = useAuth();
    ...
    /** 再点会话标签退出电脑或项目历史范围；其他标签继续保留原有页面记忆。 */
    const handleTabPress = React.useCallback((tab: TabType) => {
        if (tab === 'sessions' && pathname === '/' && [params.machineId, params.projectId, params.serverId, params.sourceKey].some((value) => value !== undefined)) {
            router.setParams({ machineId: undefined, projectId: undefined, serverId: undefined, sourceKey: undefined });
            return;
        }
        ...
        // 电脑与个人设置由路由记忆，不写入桌面共享的旧 tab KV。
        if (tab !== 'settings' && tab !== 'machines') {
            fireAndForget(setActiveTab(tab));
        }
    }, [activeTab, pathname, router, setActiveTab, params.machineId, params.projectId, params.serverId, params.sourceKey]);
    ...
    // 新建统一使用会话页右上角入口，底部三栏等宽，切换时不改变按钮位置。
