...
const newSessionUnavailableSpy = vi.hoisted(() => vi.fn());
...
installNavigationShellCommonModuleMocks({
    modal: async () => ({ Modal: { alert: newSessionUnavailableSpy } }),
    router: async () => {
...
vi.mock('@/utils/platform/responsive', () => ({
    ...
    useDeviceType: () => platformState.isTablet ? 'tablet' : 'phone',
}));
...
vi.mock('@/components/sessions/directSessions/browse/PhoneSessionsOverview', async () => {
    const ReactModule = await import('react');
    const MockSessionsListWrapper = (props: Readonly<{ pathname?: string }>) => {
        ...
        return ReactModule.createElement('PhoneSessionsOverview', props);
    };
    return {
        PhoneSessionsOverview: MockSessionsListWrapper,
    };
});
...
describe('MainView sidebar actions', () => {
    ...
    it('preserves the mounted phone sessions list across session-detail route changes', async () => {
        ...
        expect(screen.tree.findAllByType('PhoneSessionsOverview')).toHaveLength(1);
        ...
        expect(sessionsListWrapperLifecycleState.mounts).toBe(1);
        expect(sessionsListWrapperLifecycleState.unmounts).toBe(0);
        const wrappers = screen.tree.findAllByType('PhoneSessionsOverview');
