...
vi.mock('expo-router', () => ({
    ...
    useGlobalSearchParams: () => navParams,
...
describe('RootLayoutRedirectGate', () => {
    ...
    it('保留未登录通知的明确会话及服务器到首页', async () => {
        authState.isAuthenticated = false;
        navParams = { id: 'same/id', serverId: 'server-b' };
        setNav('/session/same%2Fid', ['(app)', 'session', '[id]']);
        const screen = await renderScreen(<RootLayoutRedirectGate><ShellProbe counter={{ n: 0 }} /></RootLayoutRedirectGate>);
        expect(screen.findAllByType('Redirect' as never)[0].props.href).toBe('/?id=same%2Fid&serverId=server-b&authReturn=1');
        await screen.unmount();
    });
