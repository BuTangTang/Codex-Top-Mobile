...
export const RestoreQrView = React.memo(function RestoreQrView() {
    ...
    const focusLifetimeRef = React.useRef({ active: focused, target });
    // 失焦或目标变化立即废弃旧焦点生命周期，返回同一目标也不能复活旧请求。
    if (!focused || focusLifetimeRef.current.target !== target) focusLifetimeRef.current.active = false;
    if (focused && !focusLifetimeRef.current.active) focusLifetimeRef.current = { active: true, target };
    ...
    useEffect(() => {
        if (!focusLifetime.active) return;
        isCancelledRef.current = false;
        const startQRAuth = async () => {
            // 保留原取消机制，同时阻止已失焦或目标改变后的旧二维码登录回跳。
            ...
            try {
                ...
                if (!isCurrent() || !areServerProfileIdentifiersEquivalent(loginServerId, getActiveServerSnapshot().serverId)) return;
                if (!success) {
                ...
                if (credentials && isCurrent() && areServerProfileIdentifiersEquivalent(loginServerId, getActiveServerSnapshot().serverId)) {
                    const secretString = encodeBase64(credentials.secret, 'base64url');
                    await auth.login(credentials.token, secretString);
                    if (isCurrent()) {
                        const returnTo = resolveSessionAuthReturnTo(params, getActiveServerSnapshot().serverId);
                        if (params.authReturn === '1') router.replace(returnTo ?? '/');
                        else router.back();
                    }
                } else if (isCurrent()) {
                    Modal.alert(t('common.error'), t('errors.authenticationFailed'));
                    ...
                    <View style={styles.footer}>
                        <View style={styles.footerButton}>
                            <RoundButton
                                ...
                                display="inverted"
                                onPress={() => router.push(buildAuthRestoreRoute('/restore/manual', params, getActiveServerSnapshot().serverId))}
                            />
                        </View>
