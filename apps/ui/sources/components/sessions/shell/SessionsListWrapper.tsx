...
import { filterSessionListViewDataByStatus, type SessionListStatusFilter } from '@/sync/domains/session/listing/filterSessionListViewDataByStatus';
...
const SessionsListWrapperContent = React.memo((props: { pathname: string; surfaceRoutePathname: string }) => {
    ...
    const [statusFilter, setStatusFilter] = React.useState<SessionListStatusFilter>('all');
    const { directSessionsEnabled, storageKind: selectedStorageKind, setStorageKind } = useSessionListStorageKind();
    // 使用原列表的 all 投影汇总，避免把后端存储差异变成手机用户的选择。
    const storageKind = isPhone && directSessionsEnabled ? 'all' : selectedStorageKind;
    ...
    const { sessionListViewData: sourceViewData, hasHiddenInactiveSessions } = displayPaneState;
    // 使用列表原来的共享时钟；原订阅者负责新鲜度唤醒，不增加每行定时器。
    const runtimeNowMs = useSessionListRuntimeNowMs(isPhone && surfaceOwnership.dataActive && statusFilter !== 'all');
    const sessionListViewData = React.useMemo(() => sourceViewData === null ? null
        : filterSessionListViewDataByStatus(sourceViewData, isPhone ? statusFilter : 'all', runtimeNowMs),
    [sourceViewData, isPhone, statusFilter, runtimeNowMs]);
    const visibleSessionCount = !isPhone || statusFilter === 'all'
        ? displayPaneState.visibleSessionCount
        : sessionListViewData?.reduce((count, item) => count + (item.type === 'session' ? 1 : 0), 0) ?? 0;
    ...
    const storageChrome = (
        <SessionsListStorageChrome
            ...
            statusFilter={isPhone ? statusFilter : undefined}
            onSelectStatusFilter={isPhone ? setStatusFilter : undefined}
        />
    );
    ...
    } else if (isPhone && statusFilter !== 'all' && visibleSessionCount === 0) {
        content = <View style={styles.container}>
            {storageChrome}
            <View style={[styles.emptyStateContainer, { alignItems: 'center', paddingTop: 40 }]}>
                <Text testID="sessions-status-empty" style={{ color: theme.colors.text.secondary }}>{t('common.noMatches')}</Text>
            </View>
        </View>;
