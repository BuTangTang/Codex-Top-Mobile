...
const EMPTY_SESSION_COMPOSER_SUGGESTION_KINDS: readonly ComposerSuggestionKindId[] = [];
...
type SessionHeaderRightElementProps = Readonly<{
    ...
    session: Session;
    directSessionRuntime: ReturnType<typeof useDirectSessionRuntime>;
    directControl: ChatListProps['directControlFooter'];
    paneScopeId: string;
...
/** 页头复用会话唯一接管 owner，只负责更多菜单的能力展示和动作转交。 */
const SessionHeaderRightElement = React.memo(function SessionHeaderRightElement(props: SessionHeaderRightElementProps) {
    ...
    const canOfferTakeover = Boolean(props.directControl
    ...
    const canOfferNotifications = Boolean(props.directSessionRuntime.directSessionLink
        && props.directSessionRuntime.status?.machineOnline
        ...
        if (actionId === 'header.followNotifications' || actionId === 'header.unfollowNotifications') {
            if (canOfferNotifications && !props.directSessionRuntime.notificationsBusy) {
                void props.directSessionRuntime.setNotificationsEnabled(actionId === 'header.followNotifications').then((result) => {
                    if (result === false) Modal.alert(t('common.error'), t('directSessions.followUpdateFailed'));
                });
            }
            return true;
        }
        if (actionId === 'header.takeOverDirect' || actionId === 'header.takeOverPersist') {
            const control = props.directControl;
            if (canOfferTakeover && control) {
                if (actionId === 'header.takeOverDirect' && control.canTakeOverDirect) {
                    void control.onRequestTakeOverDirect?.();
                } else if (actionId === 'header.takeOverPersist' && control.canTakeOverPersist) {
                    void control.onRequestTakeOverPersist?.();
                }
            }
            return true;
        }
    ...
    const headerExtraItems = React.useMemo(() => {
        ...
        if (canOfferNotifications) {
            ...
            items.push({
                id: followed ? 'header.unfollowNotifications' : 'header.followNotifications',
                testID: 'session-header-follow-notifications',
                title: props.directSessionRuntime.notificationsBusy ? t('common.loading')
                    : followed ? t('directSessions.unfollowNotifications') : t('directSessions.followNotifications'),
                disabled: props.directSessionRuntime.notificationsBusy,
            });
        }
        // 手机和桌面都复用现有更多菜单，已有翻译键和两种接管能力保持不变。
        if (canOfferTakeover && props.directControl?.canTakeOverDirect && props.directControl.onRequestTakeOverDirect) {
            items.push({ id: 'header.takeOverDirect', testID: 'session-header-takeOverDirect', title: t('chatFooter.takeOverDirect') });
        }
        if (canOfferTakeover && props.directControl?.canTakeOverPersist && props.directControl.onRequestTakeOverPersist) {
            items.push({ id: 'header.takeOverPersist', testID: 'session-header-takeOverPersist', title: t('chatFooter.takeOverPersist') });
        }
...
const SessionAgentInputRuntimeStatusBoundary = React.memo(function SessionAgentInputRuntimeStatusBoundary({
    ...
    const { theme } = useUnistyles();
    const connectionStatus = React.useMemo(() => desktopObservationStatus ? {
        text: t(desktopObservationStatus.textKey),
        color: theme.colors.status[desktopObservationStatus.colorKey],
        dotColor: theme.colors.status[desktopObservationStatus.colorKey],
        isPulsing: false,
    } : ({
...
/** 组装会话壳，统一持有页头与正文共享的运行和接管状态。 */
/** 复用原会话装载、路由和同步所有者，手机标题按内容占位以支持大字体。 */
export const SessionView = React.memo((props: SessionViewProps) => {
    ...
    const activeRouteServer = useActiveServerSnapshot();
    ...
    const compactPhone = deviceType === 'phone' && Platform.OS !== 'web';
    ...
    const directSessionRuntime = useDirectSessionRuntime({
        ...
        // 弹层可保留正文和 Direct 元数据；只有当前聚焦页维持 viewer，后台由 runtime 统一释放。
        viewerActive: surfaceFocused,
        serverId: currentSessionRouteServerId,
    });
    // 将唯一接管 owner 提升至页头和正文的共同祖先，hook 内按会话隔离异步生命周期。
    const directSessionLink = directSessionRuntime.directSessionLink;
    // 原生桌面继续管理执行设置；手机仅发送文本，不能展示实际不会应用的配置入口。
    const inheritsDesktopSettings = directSessionLink?.providerId === 'codex'
        && directSessionRuntime.status?.runnerActive !== true;
    const directSessionTakeover = useDirectSessionTakeover({
        sessionId,
        hasWriteAccess: Boolean(session && hasSessionWriteAccess(session.accessLevel)),
        inheritsDesktopSettings,
        directSessionRuntime,
    });

    // 页头操作与页脚说明共享一个控制投影，不注册第二个接管实例。
    const directControlFooter = React.useMemo(() => {
        if (!session || isHiddenSystemSession({ metadata: session.metadata ?? null })) return null;
        ...
        return {
            ...
            externalControl: status?.externalControl,
            inheritsDesktopSettings,
    ...
    }, [directSessionLink, directSessionRuntime.status, directSessionTakeover, inheritsDesktopSettings, session]);
    ...
    /** 手机复用工作区名称与电脑标识的正式 owner，不把完整路径作为页头。 */
    const sessionWorkspacePresentation = React.useMemo(() => {
        ...
        return resolveSessionWorkspacePresentation({
            ...
            workspaceLabelsV1,
            workspacePathDisplayModeV1: compactPhone ? 'name' : workspacePathDisplayModeV1,
        });
    ...
    const routeServerOffline = !session && !routeHydrationTerminalMissing && endpointConnectivity.status === 'offline'
        && (currentSessionRouteServerId === activeRouteServer.serverId
            || areServerProfileIdentifiersEquivalent(currentSessionRouteServerId, activeRouteServer.serverId));
    /** 重试沿用已有 hydration 与请求合并，固定原会话和所属服务，避免切服后串路由。 */
    const retryOfflineRoute = React.useCallback(() => {
        if (!currentSessionRouteServerId) return;
        fireAndForget(sync.ensureSessionVisibleForMessageRoute(sessionId, {
            serverId: currentSessionRouteServerId,
            forceRefresh: true,
        }), { tag: 'SessionView.retryOfflineRoute' });
    ...
    const headerProps = useMemo(() => {
        ...
        return {
            ...
            agentId: resolveAgentIdFromSessionMetadata(headerSession.metadata)
	            ...
	            rightElement,
            // 手机已在副标题呈现真实来源，不再用胶囊压缩任务标题。
            badges: compactPhone
                ? []
                : deviceType === 'phone'
                    ? (providerBadge ? [providerBadge] : [])
                    : (providerBadge ? [storageBadge, providerBadge] : [storageBadge]),
    ...
    return (
        ...
        <SessionScreenTestIdsProvider enabled={surfaceFocused}>
            ...
            {/* 手机让标题自然占位，字体放大时不覆盖正文；大屏继续使用原叠层布局。 */}
            {showTopHeader && shouldRenderSessionSurface && (
                <View style={{
                    position: compactPhone ? 'relative' : 'absolute',
                    top: 0,
                    left: 0,
                    right: 0,
                    zIndex: 1000
            ...
            <View style={{ flex: 1, paddingTop: showTopHeader && !compactPhone ? safeAreaTopInset + headerHeight : 0 }}>
                {!session && authSurfaceState ? (
                    <SessionAuthRecoveryFallback message={authSurfaceState.message} />
                ) : routeServerOffline ? (
                    <View testID="session-route-offline" style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 32 }}>
                        <Text style={{ color: theme.colors.text.secondary, textAlign: 'center' }}>{t('newSession.notConnectedToServer')}</Text>
                        <Pressable testID="session-route-offline-retry" accessibilityRole="button" onPress={retryOfflineRoute} style={{ padding: 16 }}>
                            <Text style={{ color: theme.colors.text.primary }}>{t('common.retry')}</Text>
                        </Pressable>
                        <Pressable testID="session-route-offline-back" accessibilityRole="button" onPress={handleBackPress} style={{ padding: 16 }}>
...
/** 组装已加载会话的消息、输入与控制区域，并连接桌面设置继承和发送能力门禁。 */
function SessionViewLoaded({
    ...
    // 桌面 owner 仅接受文本；上传能力不代表桌面能消费附件元数据。
    const attachmentsUploadsEnabled = !inheritsDesktopSettings && attachmentsUploadsFeatureEnabled && attachmentsUploadsTransferAvailable;
    ...
    const armedContinuationTarget = React.useMemo(() => {
        const intent = inheritsDesktopSettings ? null : inSessionAgentPicker.armedContinuation;
    ...
    const inactiveStatusText = inactiveUi.inactiveStatusTextKey ? t(inactiveUi.inactiveStatusTextKey) : null;
      ...
      const shouldShowInput = inactiveUi.shouldShowInput && !isEncryptedSessionLocked;
        ...
        const desktopSettingsRef = React.useRef(inheritsDesktopSettings);
        desktopSettingsRef.current = inheritsDesktopSettings;
        /** 旧文件弹层回调也必须核对最新执行归属，避免向桌面纯文本流程插入附件路径。 */
        const handleAppendLinkedPath = React.useCallback((path: string) => {
            if (desktopSettingsRef.current) return;
            setDraftValue((prev) => {
                const base = prev ?? '';
        ...
        const extraActionChips = useSessionAgentInputExtraActionChips({
            ...
            isReadOnly: isReadOnly || inheritsDesktopSettings,
    ...
    // 同一允许列表控制按钮与查询，普通文本中的 @ 不受限制。
    const composerSuggestionKinds = inheritsDesktopSettings ? EMPTY_SESSION_COMPOSER_SUGGESTION_KINDS : SESSION_COMPOSER_SUGGESTION_KINDS;
    const handleAutocompleteSuggestions = React.useCallback(
        ...
        (query: string, signal: AbortSignal) => resolveSessionComposerSuggestions(sessionId, query, {
            kinds: composerSuggestionKinds,
            signal,
        }),
        [sessionId, composerSuggestionKinds],
    );
    // 接收输入框发送意图，先校验写权限，再分派待发送消息编辑或新消息提交。
    const handleAgentInputSend = useStableAgentInputOnSend((sendOptions) => {
        ...
        // 提交当前输入；桌面文本按本次能力探测选路并沿用桌面设置，草稿按实际交接处理。
        const sendComposerText = async (
            ...
            const steerWithoutConfigMetaOverrides = nonSteerableSteerWithoutConfig
                ...
                : null;
            const shouldSendReviewComments = !inheritsDesktopSettings && hasIncludedReviewCommentDrafts;
            const hasAttachments = attachmentsUploadsEnabled && attachmentDrafts.length > 0;
            const participantRecipient = inheritsDesktopSettings ? null : recipientState.recipient;
            ...
            const resolveSendDestination = (
                ...
                route,
                armedContinuation: inheritsDesktopSettings ? null : inSessionAgentPicker.armedContinuation,
                armedContinuationLocalId: inheritsDesktopSettings ? null : inSessionAgentPicker.armedContinuationLocalId,
                machineId: typeof machineId === 'string' ? machineId : null,
                ...
                const result = await submitSessionUserMessage(sessionSubmitPort, {
                    ...
                    explicitMode: nonSteerableExplicitMode
                        ?? (!forceImmediateSend && sendIntent?.deliveryIntent === 'server_pending'
                            ...
                            : undefined),
                    forceImmediate: forceImmediateSend,
                    profileId: readyForSend === 'external' ? undefined : liveComposerState.profileId,
                    resumeCapabilityOptions,
                    resumeTargetOverride: reachableMachineTarget
                        ? {
                            machineId: reachableMachineTarget.machineId,
            ...
            if (!inheritsDesktopSettings) setIsComposerSendPending(true);
            fireAndForget((async () => {
                try {
                    if (inheritsDesktopSettings) {
                        const control = directSessionRuntime.control;
                        if (!control) {
                            Modal.alert(t('errors.failedToSendMessage'), t('chatFooter.directSessionDesktopUnknown'), [{ text: t('common.ok') }]);
                            return;
                        }
                    ...
                    await submitTextToSession(() => true);
                } finally {
                    if (!inheritsDesktopSettings) setIsComposerSendPending(false);
                }
            })(), { tag: 'SessionView.sendMessage.submitSessionUserMessage' });
        };

        // 桌面仅接收当前输入文本；本地命令解析不得绕开新快照另起执行器。
        if (inheritsDesktopSettings) {
            void sendComposerText(composerMessage, composerMessage, sendOptions);
            return;
        }
    ...
    const input = shouldShowInput ? (
        <View>
            ...
            <SessionAgentInputRuntimeStatusBoundary
                ...
                composeAgentPickerOptions={inheritsDesktopSettings ? undefined : inSessionAgentPicker.composeAgentPickerOptions}
                onAgentPickerIntent={inheritsDesktopSettings ? undefined : inSessionAgentPicker.onAgentPickerIntent}
                onAgentPickerVisibilityChange={inheritsDesktopSettings ? undefined : inSessionAgentPicker.onAgentPickerVisibilityChange}
                agentPickerSelectedOptionId={inheritsDesktopSettings ? undefined : inSessionAgentPicker.agentPickerSelectedOptionId}
                ...
                onPermissionModeChange={inheritsDesktopSettings ? undefined : updatePermissionMode}
                onAcpSessionModeChange={!inheritsDesktopSettings && supportsSessionModeOverrides(liveComposerState.agentId) ? updateAcpSessionModeOverride : undefined}
                onSessionConfigOptionChange={inheritsDesktopSettings ? undefined : updateSessionConfigOptionOverride}
                acpConfigOptionOverridesOverride={optimisticSessionConfigOptionOverrides}
                modelMode={modelMode}
                onModelModeChange={inheritsDesktopSettings ? undefined : updateModelMode}
                metadata={session.metadata}
                ...
                isSendDisabled={!shouldShowInput || isResuming || isReadOnly || isUploadingAttachments || directSessionRuntime.control?.busy === true}
                isSending={isComposerSendPending || directSessionRuntime.control?.busy === true}
                ...
                autocompleteKinds={composerSuggestionKinds}
