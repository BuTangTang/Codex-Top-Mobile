...
const hydrateRouteSpy = vi.hoisted(() => vi.fn(async () => ({ kind: "retryable_failure" })));
vi.mock("@/hooks/server/useActiveServerSnapshot", () => ({ useActiveServerSnapshot: () => ({ serverId: "server-a", generation: 1 }) }));
...
vi.mock('@/sync/sync', () => ({
    sync: {
        ensureSessionVisibleForMessageRoute: hydrateRouteSpy,
...
describe('SessionView (data ready gating)', () => {
    ...
    it('shows offline recovery while cold hydration is pending and keeps scoped retry and back usable', async () => {
        sessionState = null;
        endpointConnectivityStatus = 'offline';
        deviceTypeState.value = 'phone';
        const { SessionView } = await sessionViewModulePromise;
        const screen = await renderScreen(<AppPaneProvider><SessionView id="s1" routeServerId="server-a"
            routeHydrationState={{ kind: 'loading', sessionId: 's1', serverId: 'server-a', reason: 'cold' }} /></AppPaneProvider>);
        expect(screen.findAllByTestId('session-route-loading')).toHaveLength(0);
        expect(screen.getTextContent()).toContain('newSession.notConnectedToServer');
        await pressTestInstanceAsync(screen.findByTestId('session-route-offline-retry')!);
        expect(hydrateRouteSpy).toHaveBeenCalledWith('s1', { serverId: 'server-a', forceRefresh: true });
        await pressTestInstanceAsync(screen.findByTestId('session-route-offline-back')!);
        expect(routerBackSpy).toHaveBeenCalled();
    });

    it('preserves the hydrated composer while the server is offline', async () => {
        endpointConnectivityStatus = 'offline';
        const { SessionView } = await sessionViewModulePromise;
        const screen = await renderScreen(<AppPaneProvider><SessionView id="s1" /></AppPaneProvider>);
        expect(screen.findAllByTestId('session-composer-input')).toHaveLength(1);
        expect(screen.findAllByTestId('session-route-offline')).toHaveLength(0);
    });

    it('does not replace another server cold load with the active server offline state', async () => {
        sessionState = null;
        endpointConnectivityStatus = 'offline';
        const { SessionView } = await sessionViewModulePromise;
        const screen = await renderScreen(<AppPaneProvider><SessionView id="s1" routeServerId="other-server"
            routeHydrationState={{ kind: 'loading', sessionId: 's1', serverId: 'other-server', reason: 'cold' }} /></AppPaneProvider>);
        expect(screen.findAllByTestId('session-route-loading')).toHaveLength(1);
        expect(screen.findAllByTestId('session-route-offline-retry')).toHaveLength(0);
    });
