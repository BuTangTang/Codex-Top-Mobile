...
const deviceState = vi.hoisted(() => ({ phone: false }));
vi.mock('@/utils/platform/responsive', () => ({ useDeviceType: () => deviceState.phone ? 'phone' : 'desktop' }));
...
describe('SessionsListWrapper (empty state)', () => {
    ...
    it('filters unified phone sessions without changing desktop source and shows no matches instead of onboarding', async () => {
        deviceState.phone = true;
        featureDecisionState.enabled = true;
        storageKindState.storageKind = 'direct';
        const blocked = { type: 'session', session: { id: 'blocked', pendingBlockedCount: 1, active: false, presence: 0, metadata: null } };
        const unknown = { type: 'session', session: { id: 'unknown', active: false, presence: 0, metadata: null } };
        sessionListState.data = [blocked, unknown];
        const screen = await renderSessionsListWrapper();
        expect(sessionListState.storageKinds.every((kind) => kind === 'all')).toBe(true);
        expect(screen.findByType('SessionsListContent' as any).props.storageKind).toBe('all');
        await act(async () => { screen.findByType('SessionsListStorageChrome' as any).props.onSelectStatusFilter('attention'); });
        expect(screen.findByType('SessionsListContent' as any).props.data).toEqual([blocked]);
        expect(storageKindState.setStorageKind).not.toHaveBeenCalled();
        await act(async () => { screen.findByType('SessionsListStorageChrome' as any).props.onSelectStatusFilter('working'); });
        expect(screen.findAllByType('SessionGettingStartedGuidance' as any)).toHaveLength(0);
        expect(screen.findAllByType('SessionsListContent' as any)).toHaveLength(0);
        expect(screen.findByTestId('sessions-status-empty')).toBeTruthy();
        await act(async () => { screen.findByType('SessionsListStorageChrome' as any).props.onSelectStatusFilter('all'); });
