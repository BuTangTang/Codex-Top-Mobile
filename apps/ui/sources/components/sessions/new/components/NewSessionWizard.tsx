...
import { NewSessionPhoneForm } from './NewSessionPhoneForm';
...
export const NewSessionWizard = React.memo(function NewSessionWizard(props: NewSessionWizardProps) {
    ...
    // 增强向导与简洁模式共享手机表单，只替换布局，不切换用户保存的模式。
    if (Platform.OS === 'android' && isMobileLayoutWidth(windowWidth)) {
        return <NewSessionPhoneForm
            ...
            launch={{
                ...
                handleAgentClick: handleAgentInputAgentClick,
                agentPickerOptions: props.agent.agentPickerOptions ?? backendOptions.map((option) => {
                    ...
                    const disabledReason = !compatible
                        ...
                        : !selectable
                            ...
                            : null;
                    return { id: option.agentId, label: t(option.titleKey), subtitle: disabledReason ?? t(option.subtitleKey), disabled: Boolean(disabledReason) };
                }),
                agentPickerSelectedOptionId: props.agent.agentPickerSelectedOptionId,
                onAgentPickerSelect: props.agent.onAgentPickerSelect ?? ((id) => {
                    // 沿用向导的实际候选与选择回调，不把手机端指向已隐藏的滚动区。
                    const option = backendOptions.find((option) => option.agentId === id);
                    if (option && props.agent.isAgentSelectable(option.agentId)) props.agent.setAgentType(option.agentId);
                }),
                useProfiles: props.profiles.useProfiles,
                selectedProfileId: props.profiles.selectedProfileId,
                profilePopover,
                permissionMode: props.agent.permissionMode,
                handlePermissionModeChange: props.agent.handlePermissionModeChange,
                modelMode: props.agent.modelMode,
                setModelMode: props.agent.setModelMode,
                modelOptions: props.agent.modelOptions,
            }}
            topContent={props.composerTopContent}
        />;
    }
