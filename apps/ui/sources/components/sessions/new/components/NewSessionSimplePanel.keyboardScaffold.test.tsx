...
vi.mock('@/components/ui/popover', () => ({
    // 测试环境没有原生窗口测量；保留实际选择面板，仅替代弹层门户边界。
    MODAL_AWARE_FLOATING_POPOVER_PORTAL_OPTIONS: {},
    Popover: (props: { open: boolean; children: (args: { maxHeight: number }) => React.ReactNode }) => props.open ? props.children({ maxHeight: 500 }) : null,
...
describe('NewSessionSimplePanel keyboard scaffold integration', () => {
    ...
    it('shows and changes the restored non-Codex target and profile before submission', async () => {
        testState.platformOs = 'android';
        const { NewSessionSimplePanel } = await import('./NewSessionSimplePanel');
        const onSelect = vi.fn();
        const props = { ...createFloatingPanelProps(), agentType: 'claude' as const,
            agentPickerOptions: [{ id: 'claude', label: 'Claude' }, { id: 'codex', label: 'Codex' }],
            agentPickerSelectedOptionId: 'claude', onAgentPickerSelect: onSelect,
            useProfiles: true, selectedProfileId: 'saved-profile',
            profilePopover: { renderContent: () => React.createElement('SavedProfilePicker') },
        };
        const screen = await renderScreen(<NewSessionSimplePanel {...props} />);
        const agent = screen.findByTestId('new-session-phone-agent');
        expect(screen.getTextContent()).toContain('Claude');
        expect(screen.getTextContent()).toContain('saved-profile');
        await act(async () => { agent!.props.onPress(); });
        const picker = screen.findAllByType((await import('@/components/sessions/agentInput/components/AgentInputChipPickerPanel')).AgentInputChipPickerPanel)[0];
        await act(async () => { picker.props.onSelect('codex'); });
        expect(onSelect).toHaveBeenCalledWith('codex');
        await act(async () => { screen.findByTestId('new-session-phone-profile')!.props.onPress(); });
        expect(screen.findAllByType('SavedProfilePicker')).toHaveLength(1);
    });

    it('uses the compact Android form and keeps the same draft store and submit guard', async () => {
        testState.platformOs = 'android';
        const { NewSessionSimplePanel } = await import('./NewSessionSimplePanel');
        const baseProps = createFloatingPanelProps();
        const props = { ...baseProps, setSessionPrompt: (text: string) => baseProps.promptStore.setPrompt(text) };
        props.promptStore.setPrompt('Saved draft');
        const screen = await renderScreen(<NewSessionSimplePanel {...props} canCreate={false} />);
        expect(screen.findByTestId('new-session-phone-form')).not.toBeNull();
        const input = screen.findByTestId('new-session-phone-prompt');
        expect(input?.props.value).toBe('Saved draft');
        expect(screen.findByTestId('new-session-phone-submit')?.props.disabled).toBe(true);
        await act(async () => { input!.props.onChangeText('Updated draft'); });
        expect(props.promptStore.getPrompt()).toBe('Updated draft');
        await act(async () => { screen.tree.update(<NewSessionSimplePanel {...props} canCreate={false} />); });
        expect(screen.findByTestId('new-session-phone-prompt')?.props.value).toBe('Updated draft');
    });
