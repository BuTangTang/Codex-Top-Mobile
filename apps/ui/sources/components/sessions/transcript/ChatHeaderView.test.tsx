...
vi.mock('@/utils/platform/responsive', () => ({
    useHeaderHeight: () => 44,
    useDeviceType: () => headerMocks.deviceType,
}));
...
describe('ChatHeaderView', () => {
    ...
    it('keeps phone title and machine subtitle while omitting decorative identity', async () => {
        headerMocks.deviceType = 'phone';
        const { ChatHeaderView } = await import('./ChatHeaderView');
        const screen = await renderScreen(<ChatHeaderView title="Original conversation" subtitle="Office Mac · Project" avatarId="sample" />);
        expect(screen.getTextContent()).toContain('Original conversation');
