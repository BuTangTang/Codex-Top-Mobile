...
describe('MessageView (copy button hitSlop)', () => {
    it.each(['agent-text', 'user-text'] as const)('copies only displayed citation-safe text for %s', async (kind) => {
        ...
        const message = { kind, localId: 'local-1', id: 'm1', createdAt: 1, text, isThinking: false };
        const screen = await renderScreen(<MessageView message={message} metadata={null} sessionId="s1" />);
        const copyButton = screen.findByTestId('transcript-message-copy:m1');
        expect(Boolean(copyButton)).toBe(true);
        await act(async () => { await copyButton!.props.onPress(); });
        expect(Clipboard.setStringAsync).toHaveBeenCalledWith(kind === 'agent-text' ? 'Answer' : text);
