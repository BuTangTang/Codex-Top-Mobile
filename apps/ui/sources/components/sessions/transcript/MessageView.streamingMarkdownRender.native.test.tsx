...
describe('MessageView native streaming Markdown render', () => {
    ...
    it('never reveals a streamed internal citation while preserving the visible answer', async () => {
        const { MessageViewWithSessionCommon } = await import('./MessageView');
        const visible = 'Answer';
        const renderMessage = (text: string) => (
            <MessageViewWithSessionCommon
                sessionId="s1" metadata={null} message={createAgentMessage(text)}
                messageDisplayCommon={{ ...messageDisplayCommon, transcriptStreamingSmoothingEnabled: false }}
                forkCommon={forkCommon} toolChromeCommon={toolChromeCommon} toolRouteCommon={toolRouteCommon}
                interaction={{ canSendMessages: true, canApprovePermissions: true }}
            />
        );
        const screen = await renderScreen(renderMessage(visible));
        for (const suffix of ['<oai-mem-cit', '<oai-mem-citation>\n<citation_entries>internal', '<oai-mem-citation>internal</oai-mem-citation>']) {
            await act(async () => { await screen.update(renderMessage(`${visible}\n\n${suffix}`)); });
            await flushHookEffects({ cycles: 2, turns: 2 });
            const rendered = screen.findAllByType('EnrichedMarkdownText').map(node => node.props.markdown).join('\n');
            expect(rendered.trim()).toBe(visible);
        }
    });
