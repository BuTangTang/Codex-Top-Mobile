  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <View style={[styles.userMessageBubbleAligner, isPhone ? styles.phoneUserBubbleAligner : null]}>
            <TranscriptJumpAttention
              sessionId={props.sessionId}
              routeMessageId={buildMessageRouteId(props.message)}
              seq={seq}
              radius={TRANSCRIPT_MESSAGE_HIGHLIGHT_RADIUS}
              style={[styles.userMessageBubble, isPhone ? styles.phoneUserBubble : null, isDiscarded ? styles.userMessageBubbleDiscarded : null]}
            >
...
/** 展示助手正文，渲染与复制共用只读归一化，保留存储原文及代码示例。 */
/** 展示助手回复，保持流式、思考和工具状态使用原有渲染路径。 */
function AgentTextBlock(props: {
  ...
  const unwrappedMarkdown = (!unsupportedContentMeta && props.message.isThinking)
    ? unwrapLegacyThinkingWrapper(markdownSource)
    : markdownSource;
  // 与选择/复制共用助手显示归一化，完整及流式引用都不改写原始正文。
  const markdown = unsupportedContentMeta || isStructuredOnly
    ? unwrappedMarkdown
    : stripInternalMemoryCitations(unwrappedMarkdown);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View style={isPhone && !props.message.isThinking && !isStructuredOnly ? styles.phoneAgentBubble : undefined}>
        {structuredNode}
...
const styles = StyleSheet.create((theme) => ({
  ...
  phoneUserBubbleAligner: { maxWidth: '88%' },
  phoneUserBubble: {
