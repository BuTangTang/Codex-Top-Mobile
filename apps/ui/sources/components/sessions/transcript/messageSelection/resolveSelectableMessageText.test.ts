...
describe('resolveSelectableMessageText', () => {
    it.each([
        ['complete', 'Answer\n\n<oai-mem-citation>\n<citation_entries>internal</citation_entries>\n</oai-mem-citation>', 'Answer'],
        ['unclosed', 'Answer\n\n<oai-mem-citation>\n<citation_entries>internal', 'Answer'],
        ['partial opener', 'Answer\n\n<oai-mem-cit', 'Answer'],
        ['middle block', 'Before <oai-mem-citation>internal</oai-mem-citation> after', 'Before  after'],
        ['citation only', '<oai-mem-citation>internal</oai-mem-citation>', null],
    ])('hides %s internal citations from assistant selection without changing stored text', (_name, text, expected) => {
        const message = createAgentMessage({ text });
        expect(resolveSelectableMessageText({ message, isStructuredOnly: false, hasAttachmentBlockToStrip: false }))
            .toEqual(expected === null ? null : { role: 'assistant', text: expected });
        expect(message.text).toBe(text);
        expect(resolveSelectableMessageText({ message: createUserMessage({ text }), isStructuredOnly: false, hasAttachmentBlockToStrip: false }))
            .toEqual({ role: 'user', text });
    });

    it.each([
        '```xml\n<oai-mem-citation>example</oai-mem-citation>\n```',
        '~~~~xml\n<oai-mem-citation>example\n~~~\n</oai-mem-citation>\n~~~~',
        '```xml\n<oai-mem-citation>unfinished code example',
        '    <oai-mem-citation>indented code</oai-mem-citation>',
        'Use `<oai-mem-citation>example</oai-mem-citation>` literally.',
        '> ```xml\n> <oai-mem-citation>quoted code</oai-mem-citation>\n> ```',
        '- ```xml\n  <oai-mem-citation>list code</oai-mem-citation>\n  ```',
        '> ~~~xml\n> <oai-mem-citation>quoted code</oai-mem-citation>\n> ~~~',
        '- ~~~xml\n  <oai-mem-citation>list code</oai-mem-citation>\n  ~~~',
    ])('preserves citation tags in assistant code examples: %s', (example) => {
        const text = `${example}\n\n<oai-mem-citation>internal</oai-mem-citation>`;
        const expected = example.startsWith('```xml\n<oai-mem-citation>unfinished') ? text : example;
        expect(resolveSelectableMessageText({ message: createAgentMessage({ text }), isStructuredOnly: false, hasAttachmentBlockToStrip: false }))
            .toEqual({ role: 'assistant', text: expected });
    });
