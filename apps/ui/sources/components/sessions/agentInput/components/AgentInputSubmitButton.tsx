 ...
 */
/** 复用既有短时尺寸过渡；减少动态效果时立即收敛，手机仅调整外框留白。 */
function AgentInputSubmitShape(props: Readonly<{ children: React.ReactNode; compact?: boolean }>) {
    const styles = stylesheet;
    const reducedMotion = useReducedMotionPreference();
    /** 动画时长和降级均交由既有动态效果策略决定。 */
    ...
    return (
        <Animated.View layout={layout} style={[styles.shapeClip, props.compact ? styles.compactShape : null]}>
            {props.children}
        </Animated.View>
    );
}

/** 同一提交入口负责发送、停止和语音，紧凑外观不改变原操作判定与回调。 */
