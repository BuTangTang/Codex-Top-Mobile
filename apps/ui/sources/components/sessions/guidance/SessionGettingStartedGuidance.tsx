...
const DEFER_CLI_FOLLOW_UP_VARIANTS = new Set<SessionGettingStartedGuidanceVariant>(['newSessionBlocking']);
...
function SessionGettingStartedGuidanceViewImpl(props: SessionGettingStartedGuidanceViewProps): React.ReactElement {
    ...
    // 手机只解释当前状态，电脑安装指令与扫码流程不占据日常会话首页。
    if (props.variant === 'phone') {
        const waitingForComputer = model.kind === 'connect_machine' || model.kind === 'start_daemon';
        return (
            <ScrollView
                testID="session-getting-started-scroll"
                style={styles.scrollContainer}
                contentContainerStyle={[styles.contentContainer, styles.contentContainerCentered]}
                keyboardShouldPersistTaps="handled"
            >
                <View testID={`session-getting-started-kind-${model.kind}`} />
                <CenteredInfoTile
                    testID="session-empty-state-summary"
                    titleTestID="session-empty-state-title"
                    descriptionTestID="session-empty-state-description"
                    title={model.kind === 'connect_machine' ? t('sessionGettingStarted.phoneTitle') : title}
                    description={waitingForComputer
                        ? t('sessionGettingStarted.phoneSubtitle')
                        : model.kind === 'create_session'
                            ? t('components.emptySessionsTablet.startNewSessionDescription')
                            : subtitle}
                    icon={<Icon name="desktop" size={32} color={theme.colors.text.secondary} style={{ marginBottom: 12 }} />}
                />
