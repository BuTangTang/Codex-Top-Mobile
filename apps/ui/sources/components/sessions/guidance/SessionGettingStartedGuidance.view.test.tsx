...
describe('SessionGettingStartedGuidanceView', () => {
  ...
  it('keeps phone onboarding compact without terminal or camera actions after interactions settle', async () => {
    vi.useFakeTimers();
    try {
      const { SessionGettingStartedGuidanceView } = await import('./SessionGettingStartedGuidance');
      ...
      expect(screen.findByTestId('session-getting-started-kind-connect_machine')).not.toBeNull();
      expect(screen.findByTestId('session-getting-started-cli-follow-up')).toBeNull();
      expect(screen.findAllByType('RoundButton' as any)).toHaveLength(0);
  ...
  it('keeps the real new-session action in the compact phone state', async () => {
    const { SessionGettingStartedGuidanceView } = await import('./SessionGettingStartedGuidance');
    const onStartNewSession = vi.fn();
    const screen = await renderScreen(
      <SessionGettingStartedGuidanceView variant="phone" model={{
        kind: 'create_session', targetLabel: 'Computer', serverUrl: 'https://example.test',
        serverName: 'Example', showServerSetup: false, onStartNewSession,
      }} />,
    );
    expect(screen.findByTestId('session-getting-started-cli-follow-up')).toBeNull();
    await act(async () => {
      screen.findByTestId('session-getting-started-start-new-session')!.props.onPress();
    });
