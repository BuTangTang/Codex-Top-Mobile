...
describe('MachinesSettingsView', () => {
    ...
    it('opens a native phone computer card in its original server without CLI decoration', async () => {
        platformState.os = 'android';
        const { MachinesSettingsView } = await import('./MachinesSettingsView');
        const screen = await renderScreen(<MachinesSettingsView />);
        const row = screen.findByTestId('phone-machine-srv-a-machine-a1');
        expect(row).not.toBeNull();
        await pressTestInstanceAsync(row!);
        expect(routerPushSpy).toHaveBeenCalledWith('/(app)/machine/machine-a1?serverId=srv-a');
    });
