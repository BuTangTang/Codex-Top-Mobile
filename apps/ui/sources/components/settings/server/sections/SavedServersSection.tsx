...
export function SavedServersSection(props: SavedServersSectionProps) {
    ...
    return (
        <SettingsSection compact={props.compact} title={t('server.savedServersTitle')}>
            {groups.map((group) => {
            ...
            {props.servers.map((profile) => {
                ...
                const retentionSummary = isActive || props.compact
                    ? null
                ...
                style={props.compact ? { minHeight: 48 } : undefined}
                        titleLines={props.compact ? 0 : undefined}
                        title={profile.name}
                        subtitle={subtitle}
                        subtitleLines={0}
                        icon={<Icon name="hard-drives" size={props.compact ? 22 : 16} color={theme.colors.text.secondary} />}
                        selected={isActive}
                        showChevron={false}
                        detail={isActive ? t('server.active') : isDeviceDefault ? t('server.default') : undefined}
                        onPress={!props.compact && supportsWholeRowPress ? () => props.onSwitch(profile, 'device') : undefined}
                        rightElement={props.compact ? undefined : (
                            <ItemRowActions
                                title={profile.name}
                                actions={actions}
