...
/** 手机根页是主导航目的地，其余设置页面继续使用原来的返回栈。 */
export function getSettingsStackScreenDefinitions(t: Translate, presentation: Readonly<{ phoneMainTab?: boolean }> = {}): readonly SettingsStackScreenDefinition[] {
    return SETTINGS_ROUTE_CHROME_DEFINITIONS.map((definition) => {
        ...
        if (presentation.phoneMainTab && (definition.name === 'index' || definition.name === 'machines')) {
            options.headerTitle = t(definition.name === 'machines' ? 'tabs.machines' : 'tabs.account');
